...
public:
  ...
  // unmapped.
  TXWindow(Display* dpy_, int width = 1, int height = 1, TXWindow* parent_ = nullptr, int borderWidth = 0);
  virtual ~TXWindow();
  ...
  // by the user.
  void toplevel(const char* name, TXDeleteWindowCallback* dwc = nullptr, int argc = 0, char** argv = nullptr,
                const char* windowClass = nullptr, bool iconic = false);
  ...
  void setTransientFor(Window w) {
    XSetTransientForHint(dpy, win(), w);
  }
  ...
  bool selectionOwner(Atom selection) {
    return selectionOwner_[selection];
  }
  ...
  void invalidate(int x = 0, int y = 0, int w = 0, int h = 0) {
    XClearArea(dpy, win(), x, y, w, h, True);
  }
  ...
  // bottom/right.
  void drawBevel(GC gc, int x, int y, int w, int h, int b, unsigned long middle, unsigned long tl, unsigned long br,
                 bool round = false);
  ...
  virtual void selectionNotify(XSelectionEvent* /*ev*/, Atom /*type*/, int /*format*/, int /*nitems*/, void* /*data*/) {
  }
  ...
  virtual bool selectionRequest(Window /*requestor*/, Atom /*selection*/, Atom /*target*/, Atom /*property*/) {
    return false;
  }
  ...
  static char* strEmptyToNull(char* s) {
