...
static void vncXkbProcessDeviceEvent(int screenNum, InternalEvent* event, DeviceIntPtr dev);

/* Stolen from libX11 */
static Bool XkbTranslateKeyCode(register XkbDescPtr xkb, KeyCode key, register unsigned int mods,
...
KeyCode vncKeysymToKeycode(KeySym keysym, unsigned state, unsigned* new_state) {
  ...
  for (key = xkb->min_key_code; key <= xkb->max_key_code; key++) {
     ...
     * unused keys.
     */
    for (fakeIdx = 0; fakeIdx < sizeof(fakeKeys) / sizeof(fakeKeys[0]); fakeIdx++) {
  ...
  *new_state = (state & ~(ShiftMask | level_three_mask)) | ((state & ShiftMask) ? 0 : ShiftMask) |
...
static void vncXkbProcessDeviceEvent(int screenNum, InternalEvent* event, DeviceIntPtr dev) {
   ...
   * respected.
   */
  if ((event->device_event.type == ET_KeyPress) && key_is_down(dev, event->device_event.detail.key, KEY_PROCESSED))
