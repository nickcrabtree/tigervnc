...
static XserverDesktop* desktop[MAXSCREENS] = {
    0,
};
void* vncFbptr[MAXSCREENS] = {
    0,
};
...
core::IntParameter rfbport("rfbport", "TCP port to listen for RFB protocol", 0, -1, 65535);
...
core::BoolParameter avoidShiftNumLock("AvoidShiftNumLock",
                                      ...
                                      "NumLock.",
                                      true);
core::StringListParameter allowOverride("AllowOverride",
...
static const char* defaultDesktopName() {
  ...
  size_t len = snprintf(nullptr, 0, "%s@%s (TigerVNC %s)", pwent->pw_name, hostname.data(), XVNC_VERSION);
  if (len < 0)
    return "";

  char* name = new char[len + 1];

  snprintf(name, len + 1, "%s@%s (TigerVNC %s)", pwent->pw_name, hostname.data(), XVNC_VERSION);
...
static rfb::PixelFormat vncGetPixelFormat(int scrIdx) {
  ...
  vncGetScreenFormat(scrIdx, &depth, &bpp, &trueColour, &bigEndian, &redMask, &greenMask, &blueMask);
  ...
  return rfb::PixelFormat(bpp, depth, bigEndian, trueColour, redMax, greenMax, blueMax, redShift, greenShift,
                          blueShift);
}
      ...
      if (!desktop[scr]) {
        ...
        if (!inetd && ((const char*)rfbunixpath)[0] != '\0') {
          ...
          if (scr == 0)
            ...
            snprintf(path, sizeof(path), "%s.%d", (const char*)rfbunixpath, scr);
          path[sizeof(path) - 1] = '\0';

          listeners.push_back(new network::UnixListener(path, mode));

          vlog.info("Listening for VNC connections on %s (mode %04o)", path, mode);
        }
        ...
        if (!inetd && rfbport != -1) {
          ...
          if (!tcp_listeners.empty()) {
            ...
            vlog.info("Listening for VNC connections on %s interface(s), port %d",
                      localhostOnly ? "local" : (const char*)interface, port);
          }
        }
        ...
        desktop[scr] = new XserverDesktop(scr, listeners, name, pf, vncGetScreenWidth(), vncGetScreenHeight(),
                                          vncFbptr[scr], vncFbstride[scr]);
...
int vncConnectClient(const char* addr, int viewOnly) {
  ...
  try {
    ...
    vlog.info("Reverse connection: %s:%d%s", host.c_str(), port, viewOnly ? " (view only)" : "");
...
void vncApproveConnection(uint32_t opaqueId, int approve) {
  for (int scr = 0; scr < vncGetScreenCount(); scr++) {
    desktop[scr]->approveConnection(opaqueId, approve, "Connection rejected by local user");
  }
}
...
void vncAddChanged(int scrIdx, int nRects, const struct UpdateRect* rects) {
  for (int i = 0; i < nRects; i++) {
    desktop[scrIdx]->add_changed({{rects[i].x1, rects[i].y1, rects[i].x2, rects[i].y2}});
  }
}

void vncAddCopied(int scrIdx, int nRects, const struct UpdateRect* rects, int dx, int dy) {
  for (int i = 0; i < nRects; i++) {
    desktop[scrIdx]->add_copied({{rects[i].x1, rects[i].y1, rects[i].x2, rects[i].y2}}, {dx, dy});
  }
}

void vncSetCursorSprite(int width, int height, int hotX, int hotY, const unsigned char* rgbaData) {
  for (int scr = 0; scr < vncGetScreenCount(); scr++)
...
void vncPostScreenResize(int scrIdx, int success, int width, int height) {
  if (success) {
    ...
    try {
      desktop[scrIdx]->setFramebuffer(width, height, vncFbptr[scrIdx], vncFbstride[scrIdx]);
    } catch (std::exception& e) {
