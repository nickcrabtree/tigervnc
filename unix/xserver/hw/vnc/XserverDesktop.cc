...
core::BoolParameter rawKeyboard("RawKeyboard",
                                ...
                                "them to the current keyboard layout",
                                false);
...
XserverDesktop::XserverDesktop(int screenIndex_, std::list<network::SocketListener*> listeners_, const char* name,
                               const rfb::PixelFormat& pf, int width, int height, void* fbptr, int stride_)
    : screenIndex(screenIndex_), server(0), listeners(listeners_), shadowFramebuffer(nullptr), queryConnectId(0),
      queryConnectTimer(this) {
  format = pf;
...
void XserverDesktop::queryConnection(network::Socket* sock, const char* userName) {
  int count;
...
void XserverDesktop::setCursor(int width, int height, int hotX, int hotY, const unsigned char* rgbaData) {
  uint8_t* cursorData;
...
bool XserverDesktop::handleListenerEvent(int fd, std::list<network::SocketListener*>* sockets,
...
bool XserverDesktop::handleSocketEvent(int fd, rfb::VNCServer* sockserv, bool read, bool write) {
...
void XserverDesktop::addClient(network::Socket* sock, bool reverse, bool viewOnly) {
  vlog.debug("New client, sock %d reverse %d", sock->getFd(), reverse);
  server->addSocket(sock, reverse, viewOnly ? rfb::AccessView : rfb::AccessDefault);
  vncSetNotifyFd(sock->getFd(), screenIndex, true, false);
}
...
void XserverDesktop::getQueryConnect(uint32_t* opaqueId, const char** address, const char** username, int* timeout) {
...
void XserverDesktop::approveConnection(uint32_t opaqueId, bool accept, const char* rejectMsg) {
...
void XserverDesktop::pointerEvent(const core::Point& pos, uint16_t buttonMask) {
  vncPointerMove(pos.x + vncGetScreenX(screenIndex), pos.y + vncGetScreenY(screenIndex));
  vncPointerButtonAction(buttonMask);
}

unsigned int XserverDesktop::setScreenLayout(int fb_width, int fb_height, const rfb::ScreenSet& layout) {
  unsigned int result;
...
void XserverDesktop::grabRegion(const core::Region& region) {
  ...
  for (i = rects.begin(); i != rects.end(); i++) {
    ...
    vncGetScreenImage(screenIndex, i->tl.x, i->tl.y, i->width(), i->height(), (char*)buffer,
                      bufStride * format.bpp / 8);
    commitBufferRW(*i);
  }
}
