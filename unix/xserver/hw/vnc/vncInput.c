...
static void vncKeyboardBell(int percent, DeviceIntPtr device, void* ctrl, int class);
...
void vncInitInputDevice(void) {
  ...
  ret = AllocDevicePair(serverClient, "TigerVNC", &vncPointerDev, &vncKeyboardDev, vncPointerProc, vncKeyboardProc,
                        FALSE);

  if (ret != Success)
    FatalError("Failed to initialize TigerVNC input devices\n");

  if (ActivateDevice(vncPointerDev, TRUE) != Success || ActivateDevice(vncKeyboardDev, TRUE) != Success)
    FatalError("Failed to activate TigerVNC devices\n");

  if (!EnableDevice(vncPointerDev, TRUE) || !EnableDevice(vncKeyboardDev, TRUE))
    FatalError("Failed to activate TigerVNC devices\n");
...
void vncPointerButtonAction(int buttonMask) {
  ...
  for (i = 0; i < BUTTONS; i++) {
    if ((buttonMask ^ oldButtonMask) & (1 << i)) {
      int action = (buttonMask & (1 << i)) ? ButtonPress : ButtonRelease;
      valuator_mask_set_range(&mask, 0, 0, NULL);
      QueuePointerEvents(vncPointerDev, action, i + 1, POINTER_RELATIVE, &mask);
    }
  }
...
void vncPointerMove(int x, int y) {
  ...
  QueuePointerEvents(vncPointerDev, MotionNotify, 0, POINTER_ABSOLUTE, &mask);
...
static int vncPointerProc(DeviceIntPtr pDevice, int onoff) {
  ...
  case DEVICE_INIT:
    ...
    InitPointerDeviceStruct(pDev, map, BUTTONS, btn_labels, (PtrCtrlProcPtr)NoopDDA, GetMotionHistorySize(), 2,
                            axes_labels);
    break;
  case DEVICE_ON:
    pDev->on = TRUE;
...
static int vncKeyboardProc(DeviceIntPtr pDevice, int onoff) {
  ...
  switch (onoff) {
  case DEVICE_INIT:
    InitKeyboardDeviceStruct(pDevice, NULL, vncKeyboardBell, vncKeyboardCtrl);
    break;
  case DEVICE_ON:
    pDev->on = TRUE;
    break;
...
static void vncKeysymKeyboardEvent(KeySym keysym, int down) {
  ...
  if (!down) {
     ...
     * key presses.
     */
    LOG_DEBUG("Unexpected release of keysym XK_%s (0x%04x)", KeySymName(keysym), keysym);
    return;
  }
   ...
   */
  if ((state & ShiftMask) && ((keysym == XK_Alt_L) || (keysym == XK_Alt_R))) {
    KeyCode alt, meta;
  ...
  if (keycode == 0) {
    ...
    if (keycode == 0) {
      LOG_ERROR("Failure adding new keysym XK_%s (0x%x)", KeySymName(keysym), keysym);
      return;
    }

    LOG_INFO("Added keysym XK_%s (0x%04x) to keycode %d", KeySymName(keysym), keysym, keycode);
    ...
    keycode = vncKeysymToKeycode(keysym, state, &new_state);
    if (keycode == 0) {
      LOG_ERROR("Newly added keysym XK_%s (0x%x) cannot be generated", KeySymName(keysym), keysym);
      return;
    }
  }
   ...
   */
  if (((state & ShiftMask) != (new_state & ShiftMask)) && vncGetAvoidShiftNumLock() &&
      vncIsAffectedByNumLock(keycode)) {
    KeyCode keycode2;
    unsigned new_state2;

    LOG_DEBUG("Finding alternative to keysym XK_%s (0x%x) to avoid fake shift for numpad", KeySymName(keysym), keysym);
    ...
    for (i = 0; i < sizeof(altKeysym) / sizeof(altKeysym[0]); i++) {
      ...
      if (((state & ShiftMask) != (new_state2 & ShiftMask)) && vncIsAffectedByNumLock(keycode2))
        continue;
  ...
  if (!(state & ShiftMask) && (new_state & ShiftMask)) {
    ...
    shift_release_count = vncReleaseShift(shift_release, sizeof(shift_release) / sizeof(*shift_release));
  ...
  if (!(state & level_three_mask) && (new_state & level_three_mask)) {
    ...
    level_three_release_count =
        vncReleaseLevelThree(level_three_release, sizeof(level_three_release) / sizeof(*level_three_release));
  ...
  for (i = 0; i < 256; i++) {
    if (i == keycode)
      continue;
    if (pressedKeys[i] == keysym) {
      LOG_ERROR("Keysym XK_%s (0x%04x) generated by both keys %d and %d", KeySymName(keysym), keysym, i, keycode);
      pressedKeys[i] = NoSymbol;
    }
  }
