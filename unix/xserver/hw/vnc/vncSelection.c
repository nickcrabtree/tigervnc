...
static int vncConvertSelection(ClientPtr client, Atom selection, Atom target, Atom property, Window requestor,
                               CARD32 time, const char* data);
...
void vncHandleClipboardData(const char* data) {
  ...
  while (vncDataTargetHead != NULL) {
    ...
    rc = vncConvertSelection(vncDataTargetHead->client, vncDataTargetHead->selection, vncDataTargetHead->target,
                             vncDataTargetHead->property, vncDataTargetHead->requestor, vncDataTargetHead->time,
                             cachedData);
    if (rc != Success) {
      event.u.u.type = SelectionNotify;
      event.u.selectionNotify.time = vncDataTargetHead->time;
...
static int vncCreateSelectionWindow(void) {
  ...
  wid = FakeClientID(0);
  pWindow =
      CreateWindow(wid, pScreen->root, 0, 0, 100, 100, 0, InputOnly, 0, NULL, 0, serverClient, CopyFromParent, &result);
  if (!pWindow)
    return result;
...
static int vncOwnSelection(Atom selection) {
  ...
  if (rc == Success) {
    if (pSel->client && (pSel->client != serverClient)) {
      xEvent event = {.u.selectionClear.time = currentTime.milliseconds,
...
static int vncConvertSelection(ClientPtr client, Atom selection, Atom target, Atom property, Window requestor,
  ...
  if (data == NULL) {
    LOG_DEBUG("Selection request for %s (type %s)", NameForAtom(selection), NameForAtom(target));
  } else {
    LOG_DEBUG("Sending data for selection request for %s (type %s)", NameForAtom(selection), NameForAtom(target));
  }
  ...
  if (target == xaTARGETS) {
    Atom targets[] = {xaTARGETS, xaTIMESTAMP, xaSTRING, xaTEXT, xaUTF8_STRING};

    rc = dixChangeWindowProperty(serverClient, pWin, realProperty, XA_ATOM, 32, PropModeReplace,
                                 sizeof(targets) / sizeof(targets[0]), targets, TRUE);
    if (rc != Success)
      return rc;
  } else if (target == xaTIMESTAMP) {
    rc = dixChangeWindowProperty(serverClient, pWin, realProperty, XA_INTEGER, 32, PropModeReplace, 1,
    ...
    if (data == NULL) {
      ...
      if ((target == xaSTRING) || (target == xaTEXT)) {
        ...
        rc = dixChangeWindowProperty(serverClient, pWin, realProperty, XA_STRING, 8, PropModeReplace, strlen(latin1),
                                     latin1, TRUE);
        ...
        rc = dixChangeWindowProperty(serverClient, pWin, realProperty, xaUTF8_STRING, 8, PropModeReplace, strlen(data),
                                     data, TRUE);
        if (rc != Success)
          return rc;
      } else {
...
static int vncProcConvertSelection(ClientPtr client) {
  ...
  if (rc == Success && pSel->client == serverClient && pSel->window == wid) {
    /* cachedData will be NULL for the first request, but can then be
     * reused once we've gotten the data once from the client */
    rc = vncConvertSelection(client, stuff->selection, stuff->target, stuff->property, stuff->requestor, stuff->time,
                             cachedData);
    if (rc != Success) {
      xEvent event;
...
static void vncSelectionRequest(Atom selection, Atom target) {
  ...
  LOG_DEBUG("Requesting %s for %s selection", NameForAtom(target), NameForAtom(selection));
...
static void vncHandleSelection(Atom selection, Atom target, Atom property, Atom requestor, TimeStamp time) {
  PropertyPtr prop;
  int rc;

  rc = dixLookupProperty(&prop, pWindow, property, serverClient, DixReadAccess);
  if (rc != Success)
    return;

  LOG_DEBUG("Selection notification for %s (target %s, property %s, type %s)", NameForAtom(selection),
            NameForAtom(target), NameForAtom(property), NameForAtom(prop->type));
  ...
  if (target == xaTARGETS) {
    ...
    LOG_DEBUG("Sending clipboard to clients (%d bytes)", (int)strlen(utf8));
    ...
    LOG_DEBUG("Sending clipboard to clients (%d bytes)", (int)strlen(filtered));
...
static int vncProcSendEvent(ClientPtr client) {
  ...
  if (stuff->event.u.u.type == SelectionNotify && stuff->event.u.selectionNotify.requestor == wid) {
    TimeStamp time;
    time = ClientTimeToServerTime(stuff->event.u.selectionNotify.time);
    vncHandleSelection(stuff->event.u.selectionNotify.selection, stuff->event.u.selectionNotify.target,
                       stuff->event.u.selectionNotify.property, stuff->event.u.selectionNotify.requestor, time);
  }
...
static void vncSelectionCallback(CallbackListPtr* callbacks, void* data, void* args) {
  ...
  LOG_DEBUG("Selection owner change for %s", NameForAtom(info->selection->selection));

  if ((info->selection->selection != xaPRIMARY) && (info->selection->selection != xaCLIPBOARD))
    return;

  if ((info->selection->selection == xaPRIMARY) && !vncGetSendPrimary())
    return;
