...
static void finish_daemon(void) {
...
static pam_handle_t* run_pam(int* pamret, const char* username, const char* display) {
  pam_handle_t* pamh;
  ...
  *pamret = pam_set_item(pamh, PAM_RHOST, "203.0.113.20");
  if (*pamret != PAM_SUCCESS) {
    syslog(LOG_CRIT, "pam_set_item(PAM_RHOST) failed: %d (%s)", *pamret, pam_strerror(pamh, *pamret));
    return pamh;
  }
  ...
  *pamret = pam_set_item(pamh, PAM_XDISPLAY, display);
  ...
  if (*pamret != PAM_BAD_ITEM && *pamret != PAM_SUCCESS) {
    syslog(LOG_CRIT, "pam_set_item(PAM_XDISPLAY) failed: %d (%s)", *pamret, pam_strerror(pamh, *pamret));
    return pamh;
  }
#endif
  ...
  *pamret = pam_open_session(pamh, PAM_SILENT);
  if (*pamret != PAM_SUCCESS) {
    syslog(LOG_CRIT, "pam_open_session failed: %d (%s)", *pamret, pam_strerror(pamh, *pamret));
    return pamh;
  }
...
static int stop_pam(pam_handle_t* pamh, int pamret) {
  /* Close session */
  if (pamret == PAM_SUCCESS) {
    pamret = pam_close_session(pamh, PAM_SILENT);
    if (pamret != PAM_SUCCESS) {
      syslog(LOG_ERR, "pam_close_session failed: %d (%s)", pamret, pam_strerror(pamh, pamret));
    }
  }
...
static char* getenvp(const char* name, char** envp) {
  while (*envp) {
    size_t varlen;
    varlen = strcspn(*envp, "=");
...
static char** prepare_environ(pam_handle_t* pamh) {
...
static void switch_user(const char* username, uid_t uid, gid_t gid) {
...
static int mkdir_p(const char* path_, mode_t mode) {
...
static void redir_stdio(const char* homedir, const char* display, char** envp) {
  int fd;
  long hostlen;
...
static pid_t run_script(const char* username, const char* display, char** envp) {
  struct passwd* pwent;
  pid_t pid;
...
int main(int argc, char** argv) {
  ...
  if ((putenv("XDG_SESSION_CLASS=user") < 0) || (putenv("XDG_SESSION_TYPE=x11") < 0)) {
  ...
  snprintf(pid_file, sizeof(pid_file), "/run/vncsession-%s.pid", display);
  f = fopen(pid_file, "w");
  if (f == NULL) {
    syslog(LOG_ERR, "Failure creating pid file \"%s\": %s", pid_file, strerror(errno));
  } else {
    fprintf(f, "%ld\n", (long)getpid());
    fclose(f);
  }
  ...
  while (1) {
    ...
    if (WIFEXITED(status)) {
      if (WEXITSTATUS(status) != 0) {
        syslog(LOG_WARNING, "vncsession: vncserver exited with status=%d", WEXITSTATUS(status));
      }
      break;
    } else if (WIFSIGNALED(status)) {
      syslog(LOG_WARNING, "vncsession: vncserver was terminated by signal %d", WTERMSIG(status));
      break;
    }
  }
