...
public:
  PollingManager(Display* dpy, const Image* image, ImageFactory& factory, int offsetLeft = 0, int offsetTop = 0);
...
private:
  inline void getRow(int x, int y, int w) {
    if (w == m_width) {
      ...
      m_rowImage->get(DefaultRootWindow(m_dpy), m_offsetLeft + x, m_offsetTop + y, w, 1);
    }
  }

  inline void getColumn(int x, int y, int h) {
    m_columnImage->get(DefaultRootWindow(m_dpy), m_offsetLeft + x, m_offsetTop + y, 1, h);
  }
