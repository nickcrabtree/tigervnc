...
public:
  ...
  inline void poll(rfb::VNCServer* server) {
    m_poller->poll(server);
  }
...
protected:
  ...
  inline void grabRect(const core::Rect& r) {
    m_image->get(DefaultRootWindow(m_dpy), m_offsetLeft + r.tl.x, m_offsetTop + r.tl.y, r.width(), r.height(), r.tl.x,
                 r.tl.y);
  }
};
