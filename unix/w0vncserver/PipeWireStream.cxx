...
const pw_stream_events PipeWireStream::streamEventsHandler {
  .version = PW_VERSION_STREAM_EVENTS, .destroy = nullptr,
  .state_changed = [](void* self, enum pw_stream_state old, enum pw_stream_state state,
                      const char* error) { ((PipeWireStream*)self)->handleStreamStateChanged(old, state, error); },
  .control_info = nullptr, .io_changed = nullptr,
  .param_changed = [](void* self, uint32_t id,
                      const spa_pod* param) { ((PipeWireStream*)self)->handleStreamParamChanged(id, param); },
  .add_buffer = nullptr, .remove_buffer = nullptr,
  .process = [](void* self) { ((PipeWireStream*)self)->handleProcess(); }, .drained = nullptr,
...
PipeWireStream::PipeWireStream(int pipeWireFd_, int nodeId) : pipeWireFd(pipeWireFd_) {
...
void PipeWireStream::start(int nodeId) {
  ...
  props = pw_properties_new(PW_KEY_REMOTE_NAME, "pipewire-0", PW_KEY_MEDIA_TYPE, "Video", PW_KEY_MEDIA_CATEGORY,
  ...
  params[0] = (spa_pod*)spa_pod_builder_add_object(
      ...
      SPA_POD_CHOICE_ENUM_Id(2, SPA_VIDEO_FORMAT_RGBx, SPA_VIDEO_FORMAT_BGRx), SPA_FORMAT_VIDEO_size,
      SPA_POD_CHOICE_RANGE_Rectangle(&defaultVideoSize, &minVideoSize, &maxVideoSize), SPA_FORMAT_VIDEO_framerate,
...
void PipeWireStream::handleStreamStateChanged(enum pw_stream_state /* old */, enum pw_stream_state state,
                                              const char* error) {
  switch (state) {
...
void PipeWireStream::handleStreamParamChanged(uint32_t id, const spa_pod* param) {
  ...
  if (spa_format_parse(param, &spaFormat.media_type, &spaFormat.media_subtype) < 0)
    return;
  ...
  case SPA_MEDIA_SUBTYPE_raw:
    ...
    fbSize = SPA_RECTANGLE(spaFormat.info.raw.size.width, spaFormat.info.raw.size.height);
  ...
  if (!fbSize.width || !fbSize.height) {
    pw_stream_set_error(stream, -EINVAL, "invalid framebuffer size: %dx%d", fbSize.width, fbSize.height);
    return;
  }
  ...
  params[nParams++] = (spa_pod*)spa_pod_builder_add_object(
      ...
      SPA_POD_Int(fbSize.width * fbSize.height * mult), SPA_PARAM_BUFFERS_stride, SPA_POD_Int(fbStride),
      SPA_PARAM_BUFFERS_dataType, SPA_POD_CHOICE_FLAGS_Int((1 << SPA_DATA_MemFd)));

  params[nParams++] = (spa_pod*)spa_pod_builder_add_object(&builder, SPA_TYPE_OBJECT_ParamMeta, SPA_PARAM_Meta,
                                                           SPA_PARAM_META_type, SPA_POD_Id(SPA_META_Header),
                                                           SPA_PARAM_META_size, SPA_POD_Int(sizeof(spa_meta_header)));

  params[nParams++] = (spa_pod*)spa_pod_builder_add_object(
      &builder, SPA_TYPE_OBJECT_ParamMeta, SPA_PARAM_Meta, SPA_PARAM_META_type, SPA_POD_Id(SPA_META_VideoDamage),
      SPA_PARAM_META_size,
      SPA_POD_CHOICE_RANGE_Int(sizeof(struct spa_meta_region) * 16, sizeof(struct spa_meta_region) * 1,
...
#define CURSOR_META_SIZE(w, h) (sizeof(struct spa_meta_cursor) + sizeof(struct spa_meta_bitmap) + w * h * 4)
  ...
  params[nParams++] = (spa_pod*)spa_pod_builder_add_object(
      ...
      SPA_PARAM_META_size,
      SPA_POD_CHOICE_RANGE_Int(CURSOR_META_SIZE(64, 64), CURSOR_META_SIZE(1, 1), CURSOR_META_SIZE(512, 512)));
...
void PipeWireStream::handleProcess() {
  ...
  header = (spa_meta_header*)spa_buffer_find_meta_data(buffer->buffer, SPA_META_Header, sizeof(*header));
...
rfb::PixelFormat PipeWireStream::convertPixelformat(int spaFormat) {
  ...
  case SPA_VIDEO_FORMAT_BGRA:
    return rfb::PixelFormat(32, 24, true, true, 255, 255, 255, 8, 16, 24);
  case SPA_VIDEO_FORMAT_RGBx:
  case SPA_VIDEO_FORMAT_RGBA:
    return rfb::PixelFormat(32, 24, true, true, 255, 255, 255, 24, 16, 8);
  case SPA_VIDEO_FORMAT_xBGR:
  case SPA_VIDEO_FORMAT_ABGR:
    return rfb::PixelFormat(32, 24, true, true, 255, 255, 255, 0, 8, 16);
  case SPA_VIDEO_FORMAT_xRGB:
  case SPA_VIDEO_FORMAT_ARGB:
    return rfb::PixelFormat(32, 24, true, true, 255, 255, 255, 16, 8, 0);
  case SPA_VIDEO_FORMAT_RGB:
    return rfb::PixelFormat(24, 24, true, true, 255, 255, 255, 16, 8, 0);
  case SPA_VIDEO_FORMAT_BGR:
    return rfb::PixelFormat(24, 24, true, true, 255, 255, 255, 0, 8, 16);
  default:
    throw std::runtime_error(core::format("format %d not supported", spaFormat));
  }
}
