...
core::BoolParameter localCursor("experimentalPortalLocalCursor", "[EXPERIMENTAL] Render cursor locally", false);
...
RemoteDesktop::RemoteDesktop(std::function<void(int fd, uint32_t nodeId)> startPipewireCb_,
                             std::function<void(const char*)> cancelStartCb_)
    : sessionStarted(false), oldButtonMask(0), selectedDevices(0), sessionHandle(""), remoteDesktop(nullptr),
      screenCast(nullptr), session(nullptr), startPipewireCb(startPipewireCb_), cancelStartCb(cancelStartCb_) {
  remoteDesktop = new PortalProxy("org.freedesktop.portal.Desktop", "/org/freedesktop/portal/desktop",
                                  "org.freedesktop.portal.RemoteDesktop");
  screenCast = new PortalProxy("org.freedesktop.portal.Desktop", "/org/freedesktop/portal/desktop",
...
void RemoteDesktop::keyEvent(uint32_t keysym, uint32_t /* keycode */, bool down) {
  ...
  params = g_variant_new("(oa{sv}iu)", sessionHandle.c_str(), &optionsBuilder, keysym, state);
...
void RemoteDesktop::pointerEvent(int x, int y, uint16_t buttonMask) {
  ...
  g_variant_builder_init(&optionsBuilder, G_VARIANT_TYPE("a{sv}"));
  params = g_variant_new("(oa{sv}udd)", sessionHandle.c_str(), &optionsBuilder, pipewireNodeId, (double)x, (double)y);
...
void RemoteDesktop::handleButton(int32_t button, bool down) {
  ...
  g_variant_builder_init(&optionsBuilder, G_VARIANT_TYPE("a{sv}"));
  params = g_variant_new("(oa{sv}iu)", sessionHandle.c_str(), &optionsBuilder, button, down);
...
void RemoteDesktop::handleScrollWheel(int32_t button) {
  ...
  g_variant_builder_init(&optionsBuilder, G_VARIANT_TYPE("a{sv}"));
  params = g_variant_new("(oa{sv}ui)", sessionHandle.c_str(), &optionsBuilder, axis, steps);
...
void RemoteDesktop::createSession() {
  ...
  g_variant_builder_init(&optionsBuilder, G_VARIANT_TYPE("a{sv}"));
  g_variant_builder_add(&optionsBuilder, "{sv}", "handle_token", g_variant_new_string(requestHandleToken.c_str()));
  ...
  remoteDesktop->call("CreateSession", params, requestHandleToken.c_str(),
                      std::bind(&RemoteDesktop::handleCreateSession, this, std::placeholders::_1));
}
...
void RemoteDesktop::selectDevices() {
  ...
  g_variant_builder_add(&optionsBuilder, "{sv}", "types", g_variant_new_uint32(DEV_KEYBOARD | DEV_POINTER));
  g_variant_builder_add(&optionsBuilder, "{sv}", "persist_mode", g_variant_new_uint32(0));
  ...
  remoteDesktop->call("SelectDevices", params, requestHandleToken.c_str(),
                      std::bind(&RemoteDesktop::handleSelectDevices, this, std::placeholders::_1));
}
...
void RemoteDesktop::selectSources() {
  ...
  g_variant_builder_init(&optionsBuilder, G_VARIANT_TYPE_VARDICT);
  g_variant_builder_add(&optionsBuilder, "{sv}", "types", g_variant_new_uint32(SRC_MONITOR));
  g_variant_builder_add(&optionsBuilder, "{sv}", "cursor_mode", g_variant_new_uint32(cursorMode));
  ...
  screenCast->call("SelectSources", params, requestHandleToken.c_str(),
                   std::bind(&RemoteDesktop::handleSelectSources, this, std::placeholders::_1));
}
...
void RemoteDesktop::start() {
  ...
  params = g_variant_new("(osa{sv})", sessionHandle.c_str(), "", &optionsBuilder);

  remoteDesktop->call("Start", params, requestHandleToken.c_str(),
                      std::bind(&RemoteDesktop::handleStart, this, std::placeholders::_1));
}
...
void RemoteDesktop::openPipewireRemote() {
  ...
  g_dbus_proxy_call_with_unix_fd_list(
      screenCast->getProxy(), "OpenPipeWireRemote", g_variant_new("(oa{sv})", sessionHandle.c_str(), &optionsBuilder),
...
void RemoteDesktop::handleCreateSession(GVariant* parameters) {
  ...
  sessionHandleVariant = g_variant_lookup_value(result, "session_handle", G_VARIANT_TYPE_STRING);
  ...
  session = new PortalProxy("org.freedesktop.portal.Desktop", sessionHandle.c_str(), "org.freedesktop.portal.Session");
...
void RemoteDesktop::handleStart(GVariant* parameters) {
  ...
  streams_ = g_variant_lookup_value(result, "streams", G_VARIANT_TYPE_ARRAY);
  g_variant_unref(result);
...
void RemoteDesktop::handleOpenPipewireRemote(GObject* proxy, GAsyncResult* res) {
  ...
  response = g_dbus_proxy_call_with_unix_fd_list_finish(G_DBUS_PROXY(proxy), &fdList, res, &error);
  ...
  if (error) {
    std::string msg(error->message);
    g_error_free(error);
    fatal_error("%s", core::format("ScreenCast: error getting fd list:  %s", msg.c_str()).c_str());
    return;
  }
...
bool RemoteDesktop::parseStreams(GVariant* streams) {
  ...
  if (n_streams != 1) {
    vlog.error("RemoteDesktop.Start: only one stream supported, got %d", n_streams);
  }
