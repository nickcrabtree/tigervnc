...
class VncConfigWindow : public TXWindow,
...
public:
  VncConfigWindow(Display* dpy_)
      : TXWindow(dpy_, 300, 100), acceptClipboard(dpy_, "Accept clipboard from viewers", this, false, this),
        setPrimaryCB(dpy_, "Also set primary selection", this, false, this),
        sendClipboard(dpy_, "Send clipboard to viewers", this, false, this),
        sendPrimaryCB(dpy_, "Send primary selection to viewers", this, false, this), queryConnectDialog(nullptr) {
    int y = yPad;
  ...
  void handleEvent(TXWindow* /*w*/, XEvent* ev) override {
    if (ev->type == vncExtEventBase + VncExtQueryConnectNotify) {
      ...
      char* qcUser;
      int qcTimeout;
      if (XVncExtGetQueryConnect(dpy, &qcAddress, &qcUser, &qcTimeout, &queryConnectId)) {
        if (qcTimeout)
          queryConnectDialog = new QueryConnectDialog(dpy, qcAddress, qcUser, qcTimeout, this);
        if (queryConnectDialog)
  ...
  void checkboxSelect(TXCheckbox* checkbox) override {
    if (checkbox == &acceptClipboard) {
      XVncExtSetParam(dpy, ACCEPT_CUT_TEXT, acceptClipboard.checked() ? "1" : "0");
      setPrimaryCB.disabled(!acceptClipboard.checked());
    } else if (checkbox == &sendClipboard) {
      XVncExtSetParam(dpy, SEND_CUT_TEXT, sendClipboard.checked() ? "1" : "0");
      sendPrimaryCB.disabled(!sendClipboard.checked());
    } else if (checkbox == &setPrimaryCB) {
      XVncExtSetParam(dpy, SET_PRIMARY, setPrimaryCB.checked() ? "1" : "0");
    } else if (checkbox == &sendPrimaryCB) {
      XVncExtSetParam(dpy, SEND_PRIMARY, sendPrimaryCB.checked() ? "1" : "0");
    }
  }
