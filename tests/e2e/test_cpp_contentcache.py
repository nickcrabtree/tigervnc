...
def main():
    ...
    parser.add_argument("--bandwidth-threshold", type=float, default=10.0, help="Minimum bandwidth reduction percentage (default: 10). Set to 0 to disable enforcement.")
    ...
    try:
        ...
        server_viewer = VNCServer(args.display_viewer, args.port_viewer, "cpp_cc_viewerwin", artifacts, tracker, geometry="1920x1080", log_level="*:stderr:30", server_choice=server_mode)
        ...
        test_proc = run_cpp_viewer(
            binaries["cpp_viewer"], args.port_content, artifacts, tracker, "cpp_cc_test_viewer", cache_size_mb=args.cache_size, display_for_viewer=args.display_viewer, cache_dir=str(cache_dir)
        )
        ...
        if args.bandwidth_threshold > 0.0:
            if parsed.negotiated_contentcache:
                if bandwidth_reduction <= 0.0:
                    success = False
                    failures.append("ContentCache statistics were not logged even though ContentCache was negotiated")
