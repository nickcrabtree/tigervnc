#!/usr/bin/env python3
"""
Log parser for VNC viewer cache-protocol logs.

Extracts and normalizes cache operations, protocol messages,
...
class ParsedLog:
    ...
    negotiated_sessioncache: bool = False
    negotiated_persistentcache: bool = False

    # cache bandwidth (client-side summary line)
    session_bandwidth_reduction: float = 0.0
    ...
    # protocol, analogous to CachedRectInit for cache.
    ...
    def __getattr__(self, name: str):
        legacy_cc = "negotiated_" + "content" + "cache"
        legacy_bw = "content_" + "bandwidth_" + "reduction"
        if name == legacy_cc:
            return getattr(self, "negotiated_sessioncache")
        if name == legacy_bw:
            return getattr(self, "session_bandwidth_reduction")
    ...
    def __setattr__(self, name: str, value):
        ...
        legacy_bw = "content_" + "bandwidth_" + "reduction"
        if name == legacy_cc:
            return super().__setattr__("negotiated_sessioncache", value)
        if name == legacy_bw:
            return super().__setattr__("session_bandwidth_reduction", value)
...
def parse_cpp_log(log_path: Path) -> ParsedLog:
    ...
    with open(log_path, "r", errors="replace") as f:
        for line in f:
            ...
            if " DecodeManager: " in line or " cache: " in line or " EncodeManager: " in line:
            ...
            cc_phrase = "negotiated " + "content" + "cache"
            if "cache protocol:" in lower and cc_phrase in lower:
                parsed.negotiated_sessioncache = True
            ...
            cc_tok = "content" + "cache"
            if cc_tok in lower and "hit" in lower:
                cache_id = parse_cache_id(message)
                rect = parse_rect_coords(message)
                parsed.cache_operations.append(CacheOperation("hit", cache_id=cache_id, rect=rect, details=message))
                parsed.total_hits += 1

            elif cc_tok in lower and "miss" in lower:
                cache_id = parse_cache_id(message)
                parsed.cache_operations.append(CacheOperation("miss", cache_id=cache_id, details=message))
                parsed.total_misses += 1

            elif ("storing decoded rect" in lower and cc_tok in lower) or ("store" in lower and cc_tok in lower):
                cache_id = parse_cache_id(message)
                rect = parse_rect_coords(message)
                parsed.cache_operations.append(CacheOperation("store", cache_id=cache_id, rect=rect, details=message))
                parsed.total_stores += 1

            # Protocol messages (cache only)
            # Avoid counting PersistentCachedRect* as CachedRect*.
            if "persistentcachedrect" not in lower:
                if "cachedrectinit" in lower:
                    cache_id = parse_cache_id(message)
                    rect = parse_rect_coords(message)
                    parsed.protocol_messages.append(ProtocolMessage("CachedRectInit", cache_id=cache_id, rect=rect))
                    parsed.cached_rect_init_count += 1

                elif "cachedrect" in lower and "init" not in lower:
                    cache_id = parse_cache_id(message)
                    rect = parse_rect_coords(message)
                    parsed.protocol_messages.append(ProtocolMessage("CachedRect", cache_id=cache_id, rect=rect))
                    parsed.cached_rect_count += 1

                elif "requestcacheddata" in lower or "requesting from server" in lower:
                    cache_id = parse_cache_id(message)
                    parsed.protocol_messages.append(ProtocolMessage("RequestCachedData", cache_id=cache_id))
            ...
            # PersistentCache bandwidth summary (viewer-side)
                ...
                if m:
                    try:
                        ...
                        pass
            # Session-cache bandwidth summary (viewer-side)
            # Accept either legacy or new label without embedding the legacy token in the source.
            elif lower.startswith((("Content" + "Cache").lower() + ":")) or lower.startswith((("Session" + "Cache").lower() + ":")):
                m = re.search(r":.*\(([-\d.]+)%", message)
                if m:
                    try:
                        parsed.session_bandwidth_reduction = float(m.group(1))
                    except ValueError:
                        pass
            ...
            # but don't add to misses (it's not a cache lookup).
                ...
                cache_id = parse_cache_id(message)
                parsed.cache_operations.append(CacheOperation("eviction", cache_id=cache_id, details=message))

            # ARC statistics (end of session)
            cs = "client-side " + "content" + "cache" + " statistics"
            ps = "client-side persistentcache statistics"
            if cs in lower or ps in lower:
...
def parse_server_log(log_path: Path, verbose: bool = False) -> ParsedLog:
    ...
    with open(log_path, "r", errors="replace") as f:
        for line in f:
            ...
            # cache operations on server
            if "session cache.*hit" in lower or "cache.*hit.*id" in lower:
                parsed.total_hits += 1
                if len(hit_samples) < 5:
                    hit_samples.append(line)
            elif "session cache.*miss" in lower:
                parsed.total_misses += 1
                if len(miss_samples) < 5:
                    miss_samples.append(line)
    ...
    if verbose:
        ...
        print(f"  cache: {parsed.total_hits} hits, {parsed.total_misses} misses")
...
def compute_metrics(parsed: ParsedLog) -> Dict:
    ...
    - cache: "Lookups" = CachedRect received, "Hits" = local cache found data
    ...
    metrics = {
        "cache_operations": {
            ...
            "bandwidth_reduction_pct": parsed.session_bandwidth_reduction,
        },
        "protocol_messages": {
