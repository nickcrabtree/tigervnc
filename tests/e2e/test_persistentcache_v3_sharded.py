...
def check_disk_format(cache_dir: Path) -> tuple:
...
def main():
    parser = argparse.ArgumentParser(description="Test PersistentCache v3 sharded storage behavior")
    ...
    parser.add_argument("--cache-mem-mb", type=int, default=64, help="Memory cache size in MB (default: 64)")
    parser.add_argument("--session-duration", type=float, default=30.0, help="Duration of each viewer session in seconds (default: 30)")
    ...
    try:
        ...
        server_viewer = VNCServer(args.display_viewer, args.port_viewer, "v3_test_viewerwin", artifacts, tracker, geometry="1920x1080", log_level="*:stderr:30", server_choice=server_mode)
        ...
        # Check disk format
        success, msg = check_disk_format(cache_dir)
