...
def main() -> int:
    ...
    try:
        ...
        # The SIGUSR2 handler only sets a flag; the viewer processes it from the
        # socket event loop. If the scene is otherwise static, there may be no
        # incoming messages to wake that loop. Nudge the content-server pointer
        # inside the already-ignored top-left region to provoke a harmless cursor
        # update and ensure the refresh request is issued promptly.
        _warp_pointer_to_display(args.display_content, 6, 6, "content server pointer (wake refresh)")
        _warp_pointer_to_display(args.display_content, 5, 5, "content server pointer (park)")
