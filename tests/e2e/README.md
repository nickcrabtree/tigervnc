...
End-to-end validation of the cache protocol and the persistent policy in TigerVNC. Tests verify correct protocol implementation, cache hit rates, bandwidth savings, and cross-platform compatibility.

### Test Status (November 13, 2025)

**✅ C++ Viewer Tests Passing:**

- `test_cpp_session cache.py`: 63-67% hit rate, ~300 KB saved
...
**🚧 Rust Viewer Tests:** Awaiting cache/PersistentCache implementation in Rust viewer
...
- `test_cachedrect_init_propagation.py`: Test obsolete (tests old cache when PersistentCache is default)
- `run_baseline_rfb_test.py`: Outdated FBU count threshold
- `run_session cache_test.py`: Unbounded waits cause timeout
...
External test viewers connect to :999 and experience the cache protocol messages generated by :999 based on content from :998.
...
python3 tests/e2e/run_session cache_test.py

# Verbose output

python3 tests/e2e/run_session cache_test.py --verbose

# Custom duration

python3 tests/e2e/run_session cache_test.py --duration 120

# Custom displays/ports

python3 tests/e2e/run_session cache_test.py --display1 900 --port1 6800 \
...
ctest --test-dir build -R e2e_session cache -V
...
### Using WAN emulation with run_session cache_test.py

You can enable a WAN profile for the cache orchestrator via:

```bash
sudo python3 tests/e2e/run_session cache_test.py \
  --wan-profile 3g \
  --wan-dev lo \
  --duration 120
```
...
```text
[INFO] cache hit: cache_id=12345 rect=[100,100,64,64]
[INFO] cache miss: cache_id=67890, sending request
...
python3 tests/e2e/run_session cache_test.py --display1 950 --display2 951
```
...
# Check server parameters (Enablesession cache=1, session cacheMinRectSize=4096)
...
**Issue**: Server has cache hits but sends 0 `CachedRect` references to clients.
...
2. **Call from orchestrator**: Modify `run_session cache_test.py` to invoke your scenario:
...
- ✅ `log_parser.py`: Log parsing and normalization for cache events
- ✅ `comparator.py`: Tolerance-based metric comparison

### TODO: Remaining Work

- ⏳ `run_session cache_test.py`: Main orchestrator (VNC-in-VNC setup, viewer execution, comparison)
...
1. Implement the main orchestrator in `run_session cache_test.py` following the flow described above
...
For questions or issues with the e2e test framework, refer to the project's WARP.md for cache documentation and known issues.
