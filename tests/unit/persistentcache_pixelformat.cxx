...
static std::vector<uint8_t> createTestPixels(uint16_t width, uint16_t height, const rfb::PixelFormat& pf) {
  size_t bpp = pf.bpp / 8;
...
TEST(PersistentCachePixelFormat, ShiftFieldsPreservedAcrossSessions) {
  ...
  // Phase 1: Create cache, insert entry, save to disk
  {
    ...
    cache1.insert(canonicalHash, actualHash, testHash, testPixels.data(), testFormat, testWidth, testHeight, testWidth,
                  /*isPersistable*/ true);
