...
TEST(BandwidthStats, SavingsHighHitRatePersistentCache) {
  CacheProtocolStats stats;
  rfb::PixelFormat pf(32, 24, false, true, 255, 255, 255, 16, 8, 0);
...
TEST(BandwidthStats, RealisticPersistentCacheWorkloadHighHitRate) {
  CacheProtocolStats stats;
  rfb::PixelFormat pf(32, 24, false, true, 255, 255, 255, 16, 8, 0);
  ...
  for (int i = 0; i < 200; i++) {
    ...
    trackPersistentCacheRef(stats, core::Rect(0, 0, size, size), pf);
  }

  for (int i = 0; i < 20; i++) {
    trackPersistentCacheInit(stats, 500 + i * 50); // Variable compressed sizes
  }
