...
void makePixel(const rfb::PixelFormat& pf, uint8_t* buffer) {
  rfb::Pixel p;
...
void verifyPixel(const rfb::PixelFormat& dstpf, const rfb::PixelFormat& srcpf, const uint8_t* buffer) {
  rfb::Pixel p;
  int r, g, b;
  int re, ge, be;
...
TEST_P(Conv, bufferFromBuffer) {
  ...
  for (unaligned = 0; unaligned < 2; unaligned++) {
    ...
    dstpf.bufferFromBuffer(bufOut + unaligned, srcpf, bufIn + unaligned, fbArea);
    ...
    memset(bufOut, 0, sizeof(bufOut));
    dstpf.bufferFromBuffer(bufOut + unaligned, srcpf, bufIn + unaligned, fbWidth / 2, fbHeight, fbWidth, fbWidth);
...
TEST_P(Conv, bufferToFromRGB) {
  ...
  for (unaligned = 0; unaligned < 2; unaligned++) {
    ...
    memset(bufRGB, 0, sizeof(bufRGB));
    srcpf.rgbFromBuffer(bufRGB + unaligned, bufIn + unaligned, fbWidth / 2, fbWidth, fbHeight);

    memset(bufOut, 0, sizeof(bufOut));
    dstpf.bufferFromRGB(bufOut + unaligned, bufRGB + unaligned, fbWidth / 2, fbWidth, fbHeight);
