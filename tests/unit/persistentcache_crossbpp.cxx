...
TEST(PersistentCacheCrossBpp, Cache32bppBlitTo8bpp) {
  ...
  // Phase 1: Store 32bpp entry
  {
    ...
    cache.insert(canonicalHash, actualHash, testHash, pixels32.data(), pf32, testWidth, testHeight, testWidth, true);
  ...
  // Phase 2: Load and blit to 8bpp framebuffer
  {
    ...
    EXPECT_EQ(result[0], expected8) << "First pixel mismatch: got 0x" << std::hex << (int)result[0] << ", expected 0x"
                                    << (int)expected8;

    // Check all pixels
    for (int i = 0; i < testWidth * testHeight; i++) {
      EXPECT_EQ(result[i], expected8) << "Pixel " << i << " mismatch: got 0x" << std::hex << (int)result[i]
...
TEST(PersistentCacheCrossBpp, Cache32bppGreenBlitTo8bpp) {
  ...
  std::vector<uint8_t> testHash(16);
  for (int i = 0; i < 16; i++)
    testHash[i] = (uint8_t)(i + 0x30);
  uint64_t canonicalHash = 0x1234567890ABCDEFULL;

  {
    rfb::GlobalClientPersistentCache cache(16, 32, 1, cacheDir);
    cache.insert(canonicalHash, canonicalHash, testHash, pixels32.data(), pf32, testWidth, testHeight, testWidth, true);
    cache.flushDirtyEntries();
    ASSERT_TRUE(cache.saveToDisk());
  }

  {
    ...
    EXPECT_EQ(result[0], expected8) << "Green pixel mismatch: got 0x" << std::hex << (int)result[0] << ", expected 0x"
                                    << (int)expected8;
  }
...
TEST(PersistentCacheCrossBpp, Cache32bppBlueBlitTo8bpp) {
...

  {
    ...
    EXPECT_EQ(result[0], expected8) << "Blue pixel mismatch: got 0x" << std::hex << (int)result[0] << ", expected 0x"
                                    << (int)expected8;
  }
