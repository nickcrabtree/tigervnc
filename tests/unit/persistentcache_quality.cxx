...
TEST(PersistentCacheQuality, RejectLowQualityWhenHighNeeded) {
...

  {
    ...
    cache.insert(canonicalHash, actualHash8, hash8, pixels8.data(), pf8, testWidth, testHeight, testWidth, true);
  ...
  // Reload and test filtered lookups
  {
    ...
    EXPECT_EQ(cached, nullptr) << "32bpp filter should reject 8bpp entry - upscaling causes quality loss";
  }
...
TEST(PersistentCacheQuality, PreferHigherQualityWhenBothExist) {
...

  {
    ...
    cache.insert(canonicalHash, actualHash8, hash8, pixels8.data(), pf8, testWidth, testHeight, testWidth, true);
    cache.insert(canonicalHash, actualHash32, hash32, pixels32.data(), pf32, testWidth, testHeight, testWidth, true);
  ...
  // Reload and verify higher quality is preferred
  {
    ...
    EXPECT_EQ(cached->format.bpp, 32) << "Should prefer 32bpp entry when both exist";
  }
...
TEST(PersistentCacheQuality, PreferLosslessOverLossySameDepth) {
...

  {
    ...
    cache.insert(canonicalHash, actualHashLossy, hashLossy, pixelsLossy.data(), pf32, testWidth, testHeight, testWidth,
                 true);

    // Insert lossless entry second
    cache.insert(canonicalHash, canonicalHash, hashLossless, pixelsLossless.data(), pf32, testWidth, testHeight,
                 testWidth, true);
...

  {
    ...
    EXPECT_TRUE(cached->isLossless()) << "Expected lossless entry (actual==canonical) but got lossy";
...
TEST(PersistentCacheQuality, QualityCodePersistence) {
...

  {
    ...
    cache.insert(canon8, actual8, hash8, pixels8.data(), pf8, testWidth, testHeight, testWidth, true);

    // 16bpp lossy
    cache.insert(canon16, actual16, hash16, pixels16.data(), pf16, testWidth, testHeight, testWidth, true);

    // 32bpp lossless (actual == canonical)
    cache.insert(canon32lossless, canon32lossless, hash32ll, pixels32.data(), pf32, testWidth, testHeight, testWidth,
                 true);

    // 32bpp lossy
    cache.insert(canon32lossy, actual32, hash32lossy, pixels32.data(), pf32, testWidth, testHeight, testWidth, true);
