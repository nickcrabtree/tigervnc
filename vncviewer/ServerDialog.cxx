...
ServerDialog::ServerDialog() : Fl_Window(450, 0, "TigerVNC") {
  int x, y, x2;
  Fl_Button* button;
  Fl_Box* divider;
  ...
  serverName = new Fl_Input_Choice(LBLLEFT(x, y, w() - OUTER_MARGIN * 2, INPUT_HEIGHT, _("VNC server:")));
...
void ServerDialog::run(const char* servername, char* newservername) {
  ...
  try {
    ...
    for (const std::string& entry : dialog.serverHistory)
      fltk_menu_add(dialog.serverName->menubutton(), entry.c_str(), 0, nullptr);
...
void ServerDialog::handleLoad(Fl_Widget* /*widget*/, void* data) {
  ...
  Fl_File_Chooser* file_chooser = new Fl_File_Chooser(dialog->usedDir.c_str(), _("TigerVNC configuration (*.tigervnc)"),
  ...
  while (1) {
    ...
    if (f) {
      ...
      int overwrite_choice =
          fl_choice(_("%s already exists. Do you want to overwrite?"), _("Overwrite"), _("No"), nullptr, filename);
