...
void Surface::draw(int src_x, int src_y, int dst_x, int dst_y, int dst_w, int dst_h) {
  ...
  if (!BitBlt(fl_gc, dst_x, dst_y, dst_w, dst_h, dc, src_x, src_y, SRCCOPY)) {
...
void Surface::blend(int /*src_x*/, int /*src_y*/, int /*dst_x*/, int /*dst_y*/, int /*dst_w*/, int /*dst_h*/,
                    int /*a*/) {
...
void Surface::blend(Surface* dst, int src_x, int src_y, int dst_x, int dst_y, int dst_w, int dst_h, int a) {
  ...
  if (!AlphaBlend(dstdc, dst_x, dst_y, dst_w, dst_h, srcdc, src_x, src_y, dst_w, dst_h, blend)) {
    // If the desktop we're rendering to is inactive (like when the screen
    // is locked or the UAC is active), then GDI calls will randomly fail.
    // This is completely undocumented so we have no idea how best to deal
    // with it. For now, we've only seen this error and for this function
...
void Surface::alloc() {
  ...
  bitmap = CreateDIBSection(nullptr, (BITMAPINFO*)&bih, DIB_RGB_COLORS, (void**)&data, nullptr, 0);
  if (!bitmap)
