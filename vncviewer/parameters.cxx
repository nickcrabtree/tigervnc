...
core::IntParameter pointerEventInterval("PointerEventInterval",
                                        "Time in milliseconds to rate-limit successive "
                                        "pointer events",
                                        17, 0, INT_MAX);
core::BoolParameter emulateMiddleButton("EmulateMiddleButton",
                                        "Emulate middle mouse button by pressing left "
                                        "and right mouse buttons simultaneously",
                                        false);
core::BoolParameter dotWhenNoCursor("DotWhenNoCursor",
                                    "[DEPRECATED] Show the dot cursor when the server "
                                    "sends an invisible cursor",
                                    false);
core::BoolParameter alwaysCursor("AlwaysCursor",
                                 "Show the local cursor when the server sends an "
                                 "invisible cursor",
                                 false);
core::EnumParameter cursorType("CursorType",
                               "Specify which cursor type the local cursor should be. "
                               "Should be either Dot or System",
                               {"Dot", "System"}, "Dot");

core::BoolParameter alertOnFatalError("AlertOnFatalError",
                                      "Give a dialog on connection problems rather than "
                                      "exiting immediately",
                                      true);

core::BoolParameter reconnectOnError("ReconnectOnError",
                                     "Give a dialog on connection problems rather than "
                                     "exiting immediately and ask for a reconnect.",
                                     true);

core::StringParameter passwordFile("PasswordFile", "Password file for VNC authentication", "");
core::AliasParameter passwd("passwd", "Alias for PasswordFile", &passwordFile);

core::BoolParameter autoSelect("AutoSelect",
                               "Auto select pixel format and encoding. Default if "
                               "PreferredEncoding and FullColor are not specified.",
                               true);
core::BoolParameter fullColour("FullColor", "Use full color", true);
core::AliasParameter fullColourAlias("FullColour", "Alias for FullColor", &fullColour);
core::IntParameter lowColourLevel("LowColorLevel",
                                  "Color level to use on slow connections. "
                                  "0 = Very Low, 1 = Low, 2 = Medium",
                                  2, 0, 2);
core::AliasParameter lowColourLevelAlias("LowColourLevel", "Alias for LowColorLevel", &lowColourLevel);
core::EnumParameter preferredEncoding("PreferredEncoding",
                                      ...
                                      {"Tight", "ZRLE", "Hextile",
                                       ...
                                       "Raw"},
                                      "Tight");
core::BoolParameter customCompressLevel("CustomCompressLevel",
                                        "Use custom compression level. Default if "
                                        "CompressLevel is specified.",
                                        false);
core::IntParameter compressLevel("CompressLevel", "Use specified compression level 0 = Low, 9 = High", 2, 0, 9);
core::BoolParameter noJpeg("NoJPEG", "Disable lossy JPEG compression in Tight encoding.", false);
core::IntParameter qualityLevel("QualityLevel", "JPEG quality level. 0 = Low, 9 = High", 8, 0, 9);

core::BoolParameter maximize("Maximize", "Maximize viewer window", false);
core::BoolParameter fullScreen("FullScreen", "Enable full screen", false);
core::EnumParameter fullScreenMode("FullScreenMode",
                                   "Specify which monitors to use when in full screen. "
                                   "Should be either Current, Selected or All",
                                   {"Current", "Selected", "All"}, "Current");

core::BoolParameter fullScreenAllMonitors("FullScreenAllMonitors",
...
MonitorIndicesParameter fullScreenSelectedMonitors("FullScreenSelectedMonitors",
                                                   ...
                                                   {1});
core::StringParameter desktopSize("DesktopSize",
                                  "Reconfigure desktop size on the server on connect (if "
                                  "possible)",
                                  "");
core::StringParameter geometry("geometry", "Specify size and position of viewer window", "");

core::BoolParameter listenMode("listen", "Listen for connections from VNC servers", false);

core::BoolParameter remoteResize("RemoteResize",
                                 "Dynamically resize the remote desktop size as the size "
                                 "of the local client window changes. (Does not work "
                                 "with all servers)",
                                 true);

core::BoolParameter viewOnly("ViewOnly", "Don't send any mouse or keyboard events to the server", false);
core::BoolParameter shared("Shared",
                           "Don't disconnect other viewers upon connection - "
                           "share the desktop instead",
                           false);

core::BoolParameter acceptClipboard("AcceptClipboard", "Accept clipboard changes from the server", true);
core::BoolParameter sendClipboard("SendClipboard", "Send clipboard changes to the server", true);
#if !defined(WIN32) && !defined(__APPLE__)
core::BoolParameter setPrimary("SetPrimary",
...
core::BoolParameter sendPrimary("SendPrimary",
                                ...
                                "clipboard selection",
                                true);
core::StringParameter display("display",
...
core::EnumListParameter shortcutModifiers("ShortcutModifiers",
                                          ...
                                          {"Ctrl", "Shift", "Alt", "Super", "Win", "Option", "Cmd"}, {"Ctrl", "Alt"});

core::BoolParameter fullscreenSystemKeys("FullscreenSystemKeys",
                                         "Pass special keys (like Alt+Tab) directly to "
                                         "the server when in full-screen mode.",
                                         true);

#ifndef WIN32
core::StringParameter via("via", "Gateway to tunnel via", "");
#endif

// PersistentCache parameters (cross-session, content hashes)
core::BoolParameter persistentCache("PersistentCache",
...
core::IntParameter persistentCacheDiskSize("PersistentCacheDiskSize",
                                           "Maximum size of persistent cache on disk (MB, 0=auto 2x memory)", 0, 0,
                                           INT_MAX);
...
static core::VoidParameter* readOnlyParameterArray[] = {&fullScreenAllMonitors, &dotWhenNoCursor};
...
void saveHistoryToRegKey(const std::list<std::string>& serverHistory) {
  HKEY hKey;
  LONG res = RegCreateKeyExW(HKEY_CURRENT_USER, L"Software\\TigerVNC\\vncviewer\\history", 0, nullptr,
...
static void saveToReg(const char* servername) {
  ...
  LONG res = RegCreateKeyExW(HKEY_CURRENT_USER, L"Software\\TigerVNC\\vncviewer", 0, nullptr, REG_OPTION_NON_VOLATILE,
  ...
  try {
    ...
    RegCloseKey(hKey);
    throw std::runtime_error(core::format(_("Failed to save \"%s\": %s"), "ServerName", e.what()));
  }
  ...
  for (core::VoidParameter* param : parameterArray) {
    ...
    if (param->isDefault()) {
      try {
        ...
        RegCloseKey(hKey);
        throw std::runtime_error(core::format(_("Failed to remove \"%s\": %s"), param->getName(), e.what()));
      }
      continue;
    }
    ...
    try {
      ...
      RegCloseKey(hKey);
      throw std::runtime_error(core::format(_("Failed to save \"%s\": %s"), param->getName(), e.what()));
    }
  }
...
std::list<std::string> loadHistoryFromRegKey() {
  ...
  for (index = 0;; index++) {
    ...
    try {
      ...
      vlog.error(_("Failed to read server history entry %d: %s"), (int)index, e.what());
      continue;
    }
...
static void getParametersFromReg(core::VoidParameter* parameters[], size_t parameters_len, HKEY* hKey) {
  ...
  for (size_t i = 0; i < parameters_len; i++) {
    ...
    try {
      ...
      vlog.error(_("Failed to read parameter \"%s\": %s"), parameters[i]->getName(), e.what());
    }
  }
}
...
static char* loadFromReg() {
  ...
  try {
    ...
    vlog.error(_("Failed to read parameter \"%s\": %s"), "ServerName", e.what());
    strcpy(servername, "");
  }

  getParametersFromReg(parameterArray, sizeof(parameterArray) / sizeof(core::VoidParameter*), &hKey);
  getParametersFromReg(readOnlyParameterArray, sizeof(readOnlyParameterArray) / sizeof(core::VoidParameter*), &hKey);
...
void saveViewerParameters(const char* filename, const char* servername) {
  ...
  if (!f)
    throw core::posix_error(core::format(_("Could not open \"%s\""), filepath), errno);
  ...
  if (!encodeValue(servername, encodingBuffer, buffersize)) {
    ...
    throw std::runtime_error(
        core::format(_("Failed to save \"%s\": %s"), "ServerName", _("Could not encode parameter")));
  }
  ...
  for (core::VoidParameter* param : parameterArray) {
    ...
    if (!encodeValue(param->getValueStr().c_str(), encodingBuffer, buffersize)) {
      fclose(f);
      throw std::runtime_error(
          core::format(_("Failed to save \"%s\": %s"), param->getName(), _("Could not encode parameter")));
    }
...
static bool findAndSetViewerParameterFromValue(core::VoidParameter* parameters[], size_t parameters_len, char* value,
                                               char* line) {
  const size_t buffersize = 256;
...
char* loadViewerParameters(const char* filename) {
  ...
  while (!feof(f)) {
    ...
    if (!fgets(line, sizeof(line), f)) {
      ...
      fclose(f);
      throw core::posix_error(core::format(_("Failed to read line %d in file \"%s\""), lineNr, filepath), errno);
    }
    ...
    if (strlen(line) == (sizeof(line) - 1)) {
      ...
      std::string msg = core::format(_("Failed to read line %d in "
                                       "file \"%s\""),
                                     lineNr, filepath);
      throw std::runtime_error(core::format("%s: %s", msg.c_str(), _("Line too long")));
    }
    ...
    if (lineNr == 1) {
      ...
      fclose(f);
      throw std::runtime_error(core::format(_("Configuration file %s is in an invalid format"), filepath));
    }
    ...
    try {
      if (strcasecmp(line, "ServerName") == 0) {
        ...
        invalidParameterName = findAndSetViewerParameterFromValue(
            parameterArray, sizeof(parameterArray) / sizeof(core::VoidParameter*), value, line);

        if (invalidParameterName) {
          invalidParameterName = findAndSetViewerParameterFromValue(
              readOnlyParameterArray, sizeof(readOnlyParameterArray) / sizeof(core::VoidParameter*), value, line);
        }
      }
    } catch (std::exception& e) {
      // Just ignore this entry and continue with the rest
