...
KeyboardWin32::KeyboardWin32(KeyboardHandler* handler_)
    : Keyboard(handler_), cachedHasAltGr(false), currentLayout(nullptr), altGrArmed(false), leftShiftDown(false),
...
bool KeyboardWin32::handleEvent(const void* event) {
  ...
  if ((msg->message == WM_MOUSEMOVE) || (msg->message == WM_LBUTTONDOWN) || (msg->message == WM_LBUTTONUP) ||
      (msg->message == WM_RBUTTONDOWN) || (msg->message == WM_RBUTTONUP) || (msg->message == WM_MBUTTONDOWN) ||
      (msg->message == WM_MBUTTONUP) || (msg->message == WM_XBUTTONDOWN) || (msg->message == WM_XBUTTONUP) ||
    ...
    if (altGrArmed) {
      bool altPressed =
          isExtended && (systemKeyCode == 0x38) && (vKey == VK_MENU) && ((msg->time - altGrCtrlTime) < 50);
...
uint32_t KeyboardWin32::lookupVKeyMap(unsigned vkey, bool extended, const UINT map[][3], size_t size) {
  size_t i;
...
uint32_t KeyboardWin32::translateVKey(unsigned vkey, bool extended, const unsigned char state[256]) {
  HKL layout;
  ...
  if (primary_lang == LANG_JAPANESE) {
    ret = lookupVKeyMap(vkey, extended, vkey_map_jp, ARRAY_SIZE(vkey_map_jp));
    if (ret != NoSymbol)
      return ret;
  }

  if (primary_lang == LANG_KOREAN) {
    ret = lookupVKeyMap(vkey, extended, vkey_map_ko, ARRAY_SIZE(vkey_map_ko));
    if (ret != NoSymbol)
      return ret;
  }
...
bool KeyboardWin32::hasAltGr() {
  ...
  for (UINT vkey = 0; vkey <= 0xff; vkey++) {
    ...
    ret = ToUnicode(vkey, 0, altGrState, wstr, sizeof(wstr) / sizeof(wstr[0]), 0);
    if (ret == 1) {
      cachedHasAltGr = true;
      break;
    }
