...
CConn::CConn()
    : serverPort(0), sock(nullptr), desktop(nullptr), updateCount(0), pixelCount(0),
      lastServerEncoding((unsigned int)-1), hourlyStatsTimer(this, &CConn::handleHourlyStats), bpsEstimate(20000000),
...
CConn::~CConn() {
  ...
  if (sock) {
    ...
    unsigned long long elapsedUsec = (unsigned long long)(now.tv_sec - sessionStartTime.tv_sec) * 1000000ULL +
    ...
    vlog.info("Network summary: duration=%.1fs, rx=%s, tx=%s", seconds, core::iecPrefix(rxBytes, "B").c_str(),
              core::iecPrefix(txBytes, "B").c_str());
    vlog.info("Network throughput: rx≈%d kbit/s, tx≈%d kbit/s", (int)(rxBitsPerSec / 1000.0),
...
std::string CConn::connectionInfo() {
  ...
  infoText += core::format(_("Host: %.80s port: %d"), serverHost.c_str(), serverPort);
  infoText += "\n";

  infoText += core::format(_("Size: %d x %d"), server.width(), server.height());
  infoText += "\n";
  ...
  infoText += core::format(_("Requested encoding: %s"), rfb::encodingName(getPreferredEncoding()));
  infoText += "\n";

  infoText += core::format(_("Last used encoding: %s"), rfb::encodingName(lastServerEncoding));
  infoText += "\n";

  infoText += core::format(_("Line speed estimate: %d kbit/s"), (int)(bpsEstimate / 1000));
  infoText += "\n";

  infoText += core::format(_("Protocol version: %d.%d"), server.majorVersion, server.minorVersion);
  infoText += "\n";

  infoText += core::format(_("Security method: %s"), rfb::secTypeName(csecurity->getType()));
  infoText += "\n";
...
void CConn::handleHourlyStats(core::Timer* t) {
  ...
  if (sock) {
    ...
    vlog.info("Hourly network summary: duration=%.1fs, rx=%s, tx=%s", seconds, core::iecPrefix(rxBytes, "B").c_str(),
              core::iecPrefix(txBytes, "B").c_str());
    vlog.info("Hourly network throughput: rx≈%d kbit/s, tx≈%d kbit/s", (int)(rxBitsPerSec / 1000.0),
              (int)(txBitsPerSec / 1000.0));
  }
...
bool CConn::showMsgBox(rfb::MsgBoxFlags flags, const char* title, const char* text) {
...
void CConn::setExtendedDesktopSize(unsigned reason, unsigned result, int w, int h, const rfb::ScreenSet& layout) {
  ...
  if (verificationInProgress_) {
    ...
    if (!pb) {
        ...
        for (int x = 0; x < width; x++) {
          ...
          if (memcmp(currentPixel, savedPixel, bytesPerPixel) != 0) {
            ...
            if (diffCount <= 10) {
              ...
              if (bytesPerPixel == 4) {
                snprintf(savedStr, sizeof(savedStr), "[%02x %02x %02x %02x]", savedPixel[0], savedPixel[1],
                         savedPixel[2], savedPixel[3]);
                snprintf(currentStr, sizeof(currentStr), "[%02x %02x %02x %02x]", currentPixel[0], currentPixel[1],
                         currentPixel[2], currentPixel[3]);
              } else if (bytesPerPixel == 3) {
                snprintf(savedStr, sizeof(savedStr), "[%02x %02x %02x]", savedPixel[0], savedPixel[1], savedPixel[2]);
                snprintf(currentStr, sizeof(currentStr), "[%02x %02x %02x]", currentPixel[0], currentPixel[1],
                         currentPixel[2]);
              } else {
                snprintf(savedStr, sizeof(savedStr), "[...]");
                snprintf(currentStr, sizeof(currentStr), "[...]");
              }
              vlog.error("DIFFERENCE at (%d,%d): saved=%s current=%s", x, y, savedStr, currentStr);
            }
          }
        }
      }
      ...
      if (diffCount == 0) {
        ...
        vlog.error("VERIFICATION FAILED: %zu/%zu pixels differ (%.3f%%)", diffCount, totalPixels,
...
void CConn::setCursor(int width, int height, const core::Point& hotspot, const uint8_t* data) {
...
void CConn::updatePixelFormat() {
  ...
  if (pf != server.pf()) {
    ...
    pf.print(newStr, 256);
    vlog.info(_("PIXEL FORMAT CHANGE: old=[%s] new=[%s] bpsEstimate=%llu"), oldStr, newStr,
...
void CConn::verifyFramebuffer() {
  ...
  for (int y = 0; y < height; y++) {
    memcpy(&savedFramebuffer_[y * width * bytesPerPixel], fbData + (y * stride * bytesPerPixel), width * bytesPerPixel);
  }
...
void CConn::dumpCorruptionDebugInfo() {
  ...
  if (info.is_open()) {
    ...
    if (sock) {
      ...
      unsigned long long elapsedUsec = (unsigned long long)(now_tv.tv_sec - sessionStartTime.tv_sec) * 1000000ULL +
