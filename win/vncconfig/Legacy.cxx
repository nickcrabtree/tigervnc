  ...
  try {
    ...
    if (!authHosts.empty()) {
      ...
      try {
        ...
        MsgBox(nullptr, "Unable to convert AuthHosts setting to Hosts format.", MB_ICONWARNING | MB_OK);
      }
    } else {
...
void LegacyPage::LoadUserPrefs(const RegKey& key) {
  ...
  if (key.getBool("AutoPortSelect", false)) {
    MsgBox(nullptr,
           "The AutoPortSelect setting is not supported by this release."
  ...
  if (key.getInt("QuerySetting", 2) != 2) {
    ...
    MsgBox(nullptr,
           "The QuerySetting option has been replaced by QueryConnect."
  ...
  case 0:
    regKey.setString("DisconnectAction", "None");
    break;
  case 1:
    regKey.setString("DisconnectAction", "Lock");
    break;
  case 2:
    regKey.setString("DisconnectAction", "Logoff");
    break;
  };
