...
static core::IntParameter port_number("PortNumber", "TCP/IP port on which the server will accept connections", 5900, 0,
                                      65535);
static core::StringParameter hosts("Hosts", "Filter describing which hosts are allowed access to this server", "+");
...
public:
  ...
  bool isChanged() {
    try {
      ...
      return (new_hosts != (const char*)hosts) || (localHost != isItemChecked(IDC_LOCALHOST)) ||
             (port_number != getItemInt(IDC_PORT)) || (rfb::Server::idleTimeout != getItemInt(IDC_IDLE_TIMEOUT));
    } catch (std::exception&) {
      return false;
    }
  }
