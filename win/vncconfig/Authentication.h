...
public:
  SecPage(const RegKey& rk) : SecurityPage(nullptr), regKey(rk) {
  ...
  bool onCommand(int id, int cmd) override {
    ...
    if (id == IDC_AUTH_VNC_PASSWD) {
      ...
      const char* filter = "X.509 Certificates (*.crt;*.cer;*.pem)\0*.crt;*.cer;*.pem\0All\0*.*\0";
  ...
  static void verifyVncPassword(const RegKey& regKey) {
    if (!haveVncPassword()) {
      MsgBox(nullptr,
             ...
             "The password dialog will now be shown.",
...
private:
  ...
  inline bool showFileChooser(const RegKey& /*rk*/, const char* title, const char* filter, HWND hwnd) {
    OPENFILENAME ofn;
