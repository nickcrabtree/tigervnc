...
VNCServerService::VNCServerService()
    : Service(Name), stopServiceEvent(CreateEvent(nullptr, FALSE, FALSE, nullptr)),
    ...
    if (Process32First(hSnap, &procEntry))
      do {
        ...
        if (_stricmp(procEntry.szExeFile, "winlogon.exe") == 0 &&
            ProcessIdToSessionId(procEntry.th32ProcessID, &dwLogonSessionId) && dwLogonSessionId == dwSessionId) {
...
HANDLE LaunchProcessWin(DWORD /*dwSessionId*/) {
  ...
  if (GetSessionUserTokenWin(&hToken)) {
    ...
    if (CreateProcessAsUser(hToken, nullptr, (char*)cmdLine.c_str(), nullptr, nullptr, FALSE, DETACHED_PROCESS, nullptr,
                            nullptr, &si, &pi)) {
...
DWORD VNCServerService::serviceMain(int /*argc*/, char* /*argv*/[]) {
  ...
  while (status.dwCurrentState == SERVICE_RUNNING) {
    ...
    case WAIT_TIMEOUT: {
      ...
      DWORD dwExitCode = 0;
      if (hProcess == nullptr ||
          (GetExitCodeProcess(hProcess, &dwExitCode) && dwExitCode != STILL_ACTIVE && CloseHandle(hProcess))) {
