...
DIBSectionBuffer::DIBSectionBuffer(HWND window_) : bitmap(nullptr), window(window_), device(nullptr) {}

DIBSectionBuffer::DIBSectionBuffer(HDC device_) : bitmap(nullptr), window(nullptr), device(device_) {}
...
void DIBSectionBuffer::initBuffer(const PixelFormat& pf, int w, int h) {
  ...
  if (w && h && (format.depth != 0)) {
    ...
    if (device)
      new_bitmap = ::CreateDIBSection(device, (BITMAPINFO*)&bi.bmiHeader, iUsage, (void**)&new_data, nullptr, 0);
    else
      new_bitmap =
          ::CreateDIBSection(WindowDC(window), (BITMAPINFO*)&bi.bmiHeader, iUsage, (void**)&new_data, nullptr, 0);
  ...
  if (new_bitmap) {
    ...
    format = PixelFormat(bpp, depth, false, true, redMax, greenMax, blueMax, redShift, greenShift, blueShift);
  }
}
