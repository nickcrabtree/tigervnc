...
static void LoadHooks() {
  ...
  WM_Hooks_WindowBorderChanged =
      (WM_Hooks_WMVAL_proto)(void*)GetProcAddress(hooksLibrary, "WM_Hooks_WindowBorderChanged");
  if (WM_Hooks_WindowBorderChanged == nullptr)
    goto error;
  WM_Hooks_WindowClientAreaChanged =
      (WM_Hooks_WMVAL_proto)(void*)GetProcAddress(hooksLibrary, "WM_Hooks_WindowClientAreaChanged");
  ...
  WM_Hooks_SetDiagnosticRange =
      (WM_Hooks_SetDiagnosticRange_proto)(void*)GetProcAddress(hooksLibrary, "WM_Hooks_SetDiagnosticRange");
  if (WM_Hooks_SetDiagnosticRange == nullptr)
    goto error;
#endif

  WM_Hooks_EnableRealInputs =
      (WM_Hooks_EnableRealInputs_proto)(void*)GetProcAddress(hooksLibrary, "WM_Hooks_EnableRealInputs");
...
class WMHooksThread {
public:
  WMHooksThread() : active(true), thread(&WMHooksThread::worker, this), thread_id(-1) {}
  void stop();
  DWORD getThreadId() {
    return thread_id;
  }
...
void WMHooksThread::worker() {
  ...
  while (active && GetMessage(&msg, nullptr, 0, 0)) {
    ...
    if (msg.message == WM_TIMER) {
      ...
      if (IsWindow(hwnd) && IsWindowVisible(hwnd) && !IsIconic(hwnd) && GetWindowRect(hwnd, &wrect) &&
          !IsRectEmpty(&wrect)) {
        updates[activeRgn].assign_union({{wrect.left, wrect.top, wrect.right, wrect.bottom}});
      ...
      hwnd = (HWND)msg.lParam;
      if (IsWindow(hwnd) && IsWindowVisible(hwnd) && !IsIconic(hwnd) && GetClientRect(hwnd, &wrect) &&
          !IsRectEmpty(&wrect)) {
        POINT pt = {0, 0};
        if (ClientToScreen(hwnd, &pt)) {
          updates[activeRgn].assign_union(
              {{wrect.left + pt.x, wrect.top + pt.y, wrect.right + pt.x, wrect.bottom + pt.y}});
          updateDelayTimer.start(updateDelayMs);
        }
      }
      ...
      if (IsWindow(hwnd) && IsWindowVisible(hwnd) && !IsIconic(hwnd) && GetWindowRect(hwnd, &wrect) &&
        ...
        RECT crect;
        POINT pt = {0, 0};
        if (GetClientRect(hwnd, &crect) && ClientToScreen(hwnd, &pt) && !IsRectEmpty(&crect)) {
          changed.assign_subtract({{crect.left + pt.x, crect.top + pt.y, crect.right + pt.x, crect.bottom + pt.y}});
        }
        if (!changed.is_empty()) {
      ...
      Rect r(LOWORD(msg.wParam), HIWORD(msg.wParam), LOWORD(msg.lParam), HIWORD(msg.lParam));
      if (!r.is_empty()) {
      ...
      vlog.info("DIAG msg=%x(%d) wnd=%lx", (unsigned)msg.wParam, (int)msg.wParam, (unsigned long)msg.lParam);
#endif
    }
  }
...
#ifdef _DEBUG
void rfb::win32::WMHooks::setDiagnosticRange(UINT min, UINT max) {
