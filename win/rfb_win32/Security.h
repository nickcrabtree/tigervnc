...
struct Trustee : public TRUSTEE {
  Trustee(const char* name, TRUSTEE_FORM form = TRUSTEE_IS_NAME, TRUSTEE_TYPE type = TRUSTEE_IS_UNKNOWN);
};

struct ExplicitAccess : public EXPLICIT_ACCESS {
  ExplicitAccess(const char* name, TRUSTEE_FORM type, DWORD perms, ACCESS_MODE mode, DWORD inherit = 0);
};
...
struct AccessEntries {
  ...
  void addEntry(const char* trusteeName, DWORD permissions, ACCESS_MODE mode);
...
struct SecurityDescriptorPtr : LocalMem {
  ...
  PSECURITY_DESCRIPTOR takeSD() {
    return (PSECURITY_DESCRIPTOR)takePtr();
  }
};
