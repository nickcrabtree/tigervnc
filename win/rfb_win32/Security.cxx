...
Trustee::Trustee(const char* name, TRUSTEE_FORM form, TRUSTEE_TYPE type) {
...
ExplicitAccess::ExplicitAccess(const char* name, TRUSTEE_FORM type, DWORD perms, ACCESS_MODE mode, DWORD inherit) {
...
void AccessEntries::addEntry(const char* trusteeName, DWORD permissions, ACCESS_MODE mode) {
...
void AccessEntries::addEntry(const PSID sid, DWORD permissions, ACCESS_MODE mode) {
  allocMinEntries(entry_count + 1);
  ZeroMemory(&entries[entry_count], sizeof(EXPLICIT_ACCESS));
...
Sid::SYSTEM::SYSTEM() {
  PSID sid = nullptr;
  SID_IDENTIFIER_AUTHORITY ntAuth = {SECURITY_NT_AUTHORITY};
  if (!AllocateAndInitializeSid(&ntAuth, 1, SECURITY_LOCAL_SYSTEM_RID, 0, 0, 0, 0, 0, 0, 0, &sid))
