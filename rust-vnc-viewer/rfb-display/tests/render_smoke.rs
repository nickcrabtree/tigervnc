...
fn test_framebuffer_rendering_setup() {
    ...
    framebuffer
        .fill_rect(test_rect, &[255, 128, 64, 255])
        .unwrap(); // RGBA
...
fn test_scaling_performance() {
    ...
    assert!(
        fit_duration.as_micros() < 1000,
        "Fit scaling too slow: {:?}",
        fit_duration
    );
    assert!(
        fill_duration.as_micros() < 1000,
        "Fill scaling too slow: {:?}",
        fill_duration
    );
    assert!(
        zoom_duration.as_micros() < 1000,
        "Zoom calculation too slow: {:?}",
        zoom_duration
    );
}
