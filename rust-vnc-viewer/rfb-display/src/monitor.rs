...
impl MonitorInfo {
    ...
    pub fn logical_size(&self) -> PhysicalSize<u32> {
        let (width, height) = self.dpi_config.physical_size_to_logical(self.size.width, self.size.height);
...
mod tests {
    ...
    fn test_window_placement_display() {
        ...
        assert_eq!(format!("{}", WindowPlacement::LargestMonitor), "Largest Monitor");
        assert_eq!(format!("{}", WindowPlacement::RememberLast), "Remember Last Position");
    }
    ...
    fn test_monitor_manager_with_mock_monitors() {
        ...
        manager.monitors.push(create_test_monitor("Monitor1", 0, 0, 1920, 1080, 1.0, true));
        manager.monitors.push(create_test_monitor("Monitor2", 1920, 0, 1920, 1080, 1.0, false));
        manager.primary_index = Some(0);
        
        assert_eq!(manager.monitor_count(), 2);
        assert!(manager.primary_monitor().is_some());
        assert_eq!(manager.primary_monitor().unwrap().name.as_deref(), Some("Monitor1"));
    ...
    fn test_monitor_at_point() {
        let mut manager = MonitorManager::new();
        manager.monitors.push(create_test_monitor("Left", 0, 0, 1920, 1080, 1.0, true));
        manager.monitors.push(create_test_monitor("Right", 1920, 0, 1920, 1080, 1.0, false));
        ...
        assert_eq!(manager.monitor_at_point(left_point).unwrap().name.as_deref(), Some("Left"));
        assert_eq!(manager.monitor_at_point(right_point).unwrap().name.as_deref(), Some("Right"));
    ...
    fn test_largest_monitor() {
        let mut manager = MonitorManager::new();
        manager.monitors.push(create_test_monitor("Small", 0, 0, 1280, 720, 1.0, true));
        manager.monitors.push(create_test_monitor("Large", 1280, 0, 2560, 1440, 1.0, false));
    ...
    fn test_monitor_manager_summary() {
        let mut manager = MonitorManager::new();
        manager.monitors.push(create_test_monitor("Monitor1", 0, 0, 1920, 1080, 1.0, true));
        manager.monitors.push(create_test_monitor("Monitor2", 1920, 0, 1920, 1080, 2.0, false));
