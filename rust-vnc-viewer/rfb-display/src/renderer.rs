...
impl DisplayRendererBuilder {
    ...
    pub async fn build_for_window(
        self,
        window: Arc<Window>,
    ) -> Result<DisplayRenderer, RenderError> {
...
impl DisplayRenderer {
    ...
    pub fn resize(&mut self, new_size: PhysicalSize<u32>) -> Result<(), RenderError> {
        ...
        self.viewport
            .set_window_size(new_size.width, new_size.height);
        ...
        P: PixelBuffer,
    {
        ...
        self.set_framebuffer_size(fb_width, fb_height);
        ...
        self.cursor_renderer
            .render_to_frame(frame, texture_width, texture_height);
    ...
    where
        P: PixelBuffer,
    {
        let (frame_width, frame_height) = (texture_width as i32, texture_height as i32);
    ...
    fn render_native_scale_static(
        ...
        let copy_width =
            ((fb_width as i32 - src_x as i32).min(frame_width - dst_x as i32)).max(0) as usize;
        let copy_height =
            ((fb_height as i32 - src_y as i32).min(frame_height - dst_y as i32)).max(0) as usize;
    ...
    fn render_fit_scale_static(
        ...
        let scale_params =
            crate::ScaleParams::fit(fb_width, fb_height, frame_width as u32, frame_height as u32);
    ...
    fn render_fill_scale_static(
        ...
        let scale_params =
            crate::ScaleParams::fill(fb_width, fb_height, frame_width as u32, frame_height as u32);
    ...
    fn render_scaled_static(
        ...
        let dst_start_x = scale_params.offset_x.max(0);
        let dst_start_y = scale_params.offset_y.max(0);
        ...
        for dst_y in dst_start_y..dst_end_y {
            for dst_x in dst_start_x..dst_end_x {
                ...
                let src_idx =
                    src_y as usize * stride * bytes_per_pixel + src_x as usize * bytes_per_pixel;
                let dst_idx = dst_y as usize * frame_width as usize * 4 + dst_x as usize * 4;

                if src_idx + bytes_per_pixel <= fb_data.len()
                    && dst_idx + 4 <= frame.len()
                    && bytes_per_pixel >= 3
                {
