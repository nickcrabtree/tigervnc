...
use crate::persistent_cache::{PersistentClientCache, PersistentCachedPixels};
...
impl PersistentCachedRectInitDecoder {
    pub fn new(cache: Arc<Mutex<PersistentClientCache>>) -> Self {
        Self { cache, raw: RawDecoder, copyrect: CopyRectDecoder, rre: RREDecoder,
            hextile: HextileDecoder, tight: TightDecoder::default(), zrle: ZRLEDecoder::default() }
    }
}

impl Decoder for PersistentCachedRectInitDecoder {
    fn encoding_type(&self) -> i32 { ENCODING_PERSISTENT_CACHED_RECT_INIT }
    ...
    async fn decode<R: AsyncRead + Unpin>(
        ...
        let actual = stream.read_i32().await.context("read persistent actual encoding")?;

        // Build a Rectangle for inner decoder
        let actual_rect = Rectangle { x: rect.x, y: rect.y, width: rect.width, height: rect.height, encoding: actual };

        // Dispatch
        match actual {
            ENCODING_RAW => self.raw.decode(stream, &actual_rect, pixel_format, buffer).await?,
            ENCODING_COPY_RECT => self.copyrect.decode(stream, &actual_rect, pixel_format, buffer).await?,
            ENCODING_RRE => self.rre.decode(stream, &actual_rect, pixel_format, buffer).await?,
            ENCODING_HEXTILE => self.hextile.decode(stream, &actual_rect, pixel_format, buffer).await?,
            ENCODING_TIGHT => self.tight.decode(stream, &actual_rect, pixel_format, buffer).await?,
            ENCODING_ZRLE => self.zrle.decode(stream, &actual_rect, pixel_format, buffer).await?,
            _ => anyhow::bail!("Unsupported inner encoding {} for PersistentCachedRectInit", actual),
        }
        ...
        if let Some(pixels) = buffer.get_buffer(dest_rect, &mut stride_pixels) {
            ...
            let entry = PersistentCachedPixels {
                ...
                format: buffer.pixel_format().clone(),
            ...
            cache.insert(entry);
            tracing::info!("PersistentCache STORE: rect {}x{} id={:02x?}", rect.width, rect.height, &id);
        }
