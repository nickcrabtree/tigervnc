...
use rfb_encodings::{
    PersistentCachedPixels, PersistentCachedRectDecoder, PersistentCachedRectInitDecoder,
    PersistentClientCache, PixelFormat, RfbInStream, ENCODING_PERSISTENT_CACHED_RECT,
    ENCODING_PERSISTENT_CACHED_RECT_INIT, ENCODING_RAW,
};
...
async fn persistent_cache_round_trip() {
    ...
    ref_dec
        .decode(&mut stream, &rect_ref, &pf, &mut buf)
        .await
        .unwrap();
    ...
    init_dec
        .decode(&mut stream2, &rect_init, &pf, &mut buf)
        .await
        .unwrap();
    ...
    ref_dec
        .decode(&mut stream3, &rect_ref, &pf, &mut buf)
        .await
        .unwrap();
