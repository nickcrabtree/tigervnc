...
pub fn render(app: &mut VncViewerApp, ui: &mut Ui, _ctx: &Context) {
    ...
    let (display_width, display_height) =
        if let Some((content_width, content_height)) = app.content_size() {
            let zoom = app.zoom_factor();
            (
                (content_width as f32 * zoom) as u32,
                (content_height as f32 * zoom) as u32,
            )
        } else {
    ...
    egui::ScrollArea::both()
        ...
        .show(ui, |ui| {
            ...
            // Draw border
            ui.painter()
                .rect_stroke(rect, egui::Rounding::ZERO, Stroke::new(1.0, Color32::GRAY));
            ...
            if response.hovered() && matches!(app.state(), crate::app::AppState::Connected(_)) {
                ...
                if let Some(hover_pos) = response.hover_pos() {
                    ...
                    ui.painter()
                        .circle_stroke(hover_pos, 3.0, Stroke::new(1.0, Color32::YELLOW));
                }
            }
