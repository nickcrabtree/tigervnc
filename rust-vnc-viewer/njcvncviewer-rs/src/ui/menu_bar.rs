    ...
    egui::TopBottomPanel::top("menu_bar").show(ctx, |ui| {
        egui::menu::bar(ui, |ui| {
            ...
            ui.menu_button("View", |ui| {
                if ui.checkbox(&mut app.ui_state_mut().fullscreen, "Fullscreen").clicked() {
                ...
                ui.menu_button("Scaling", |ui| {
                    ...
                    if ui.radio_value(&mut current_mode, rfb_display::ScaleMode::Native, "Native (1:1)").clicked() {
                        app.set_scale_mode(rfb_display::ScaleMode::Native);
                        ui.close_menu();
                    }
                    if ui.radio_value(&mut current_mode, rfb_display::ScaleMode::Fit, "Fit Window").clicked() {
                        app.set_scale_mode(rfb_display::ScaleMode::Fit);
                        ui.close_menu();
                    }
                    if ui.radio_value(&mut current_mode, rfb_display::ScaleMode::Fill, "Fill Window").clicked() {
            ...
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                match app.state() {
                    crate::app::AppState::Connected(server_name) => {
                        ui.colored_label(egui::Color32::GREEN, format!("🟢 Connected to {}", server_name));
                    }
