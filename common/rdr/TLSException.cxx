...
tls_error::tls_error(const char* s, int err_, int alert_) noexcept
    : std::runtime_error(core::format("%s: %s (%d)", s, strerror(err_, alert_), err_)), err(err_), alert(alert_) {}
...
const char* tls_error::strerror(int err_, int alert_) const noexcept {
  ...
  if ((alert_ != -1) && ((err_ == GNUTLS_E_WARNING_ALERT_RECEIVED) || (err_ == GNUTLS_E_FATAL_ALERT_RECEIVED)))
