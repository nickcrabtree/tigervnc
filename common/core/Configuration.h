...
public:
  ...
  std::list<VoidParameter*>::iterator begin() {
    return params.begin();
  }
  std::list<VoidParameter*>::iterator end() {
    return params.end();
  }
...
public:
  IntParameter(const char* name_, const char* desc_, int v, int minValue = INT_MIN, int maxValue = INT_MAX);
...
public:
  EnumParameter(const char* name_, const char* desc_, const std::set<const char*>& enums, const char* v);
  bool setParam(const char* value) override;
  std::string getDefaultStr() const override;
  std::string getValueStr() const override;
...
public:
  BinaryParameter(const char* name_, const char* desc_, const uint8_t* v, size_t l);
  using VoidParameter::setParam;
...
template <typename ValueType> class ListParameter : public VoidParameter {
public:
  typedef std::list<ValueType> ListType;
  typedef typename ListType::const_iterator const_iterator;

  ListParameter(const char* name_, const char* desc_, const ListType& v);
  using VoidParameter::setParam;
  bool setParam(const char* value) override;
...
public:
  IntListParameter(const char* name_, const char* desc_, const ListType& v, int minValue = INT_MIN,
...
public:
  StringListParameter(const char* name_, const char* desc_, const std::list<const char*>& v);
  ...
  public:
    ...
    const char* operator*() const {
      return (ListType::const_iterator::operator*()).c_str();
    }
  };
...
public:
  EnumListParameter(const char* name_, const char* desc_, const std::set<const char*>& enums,
                    const std::list<const char*>& v);

  class const_iterator : public ListType::const_iterator {
  public:
    const_iterator(const ListType::const_iterator& it) : ListType::const_iterator(it) {}
    const EnumListEntry operator*() const {
      return EnumListEntry(ListType::const_iterator::operator*());
    }
    const EnumListEntry operator->() const {
      return EnumListEntry(*ListType::const_iterator::operator->());
    }
