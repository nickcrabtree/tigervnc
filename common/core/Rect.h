...
struct Point {
  ...
  inline Point negate() const __attribute__((warn_unused_result)) {
    return Point(-x, -y);
  }
  ...
  inline Point translate(const Point& p) const __attribute__((warn_unused_result)) {
    return Point(x + p.x, y + p.y);
  }
  inline Point subtract(const Point& p) const __attribute__((warn_unused_result)) {
    return Point(x - p.x, y - p.y);
  }
  int x, y;
};
...
struct Rect {
  ...
  inline Rect intersect(const Rect& r) const __attribute__((warn_unused_result)) {
    Rect result;
  ...
  inline Rect union_boundary(const Rect& r) const __attribute__((warn_unused_result)) {
    if (r.is_empty())
      return *this;
    if (is_empty())
      return r;
  ...
  inline Rect translate(const Point& p) const __attribute__((warn_unused_result)) {
    return Rect(tl.translate(p), br.translate(p));
  }
  inline bool operator==(const Rect& r) const {
    return r.tl == tl && r.br == br;
  }
  inline bool operator!=(const Rect& r) const {
    return r.tl != tl || r.br != br;
  }
  ...
  inline int area() const {
    return is_empty() ? 0 : (br.x - tl.x) * (br.y - tl.y);
  }
  inline Point dimensions() const {
