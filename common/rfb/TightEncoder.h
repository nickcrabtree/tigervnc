...
protected:
  ...
  void writePixels(const uint8_t* buffer, const PixelFormat& pf, unsigned int count, rdr::OutStream* os);
...
protected:
  ...
  template <class T>
  void writeMonoRect(int width, int height, const T* buffer, int stride, const PixelFormat& pf, const Palette& palette);
  template <class T>
  void writeIndexedRect(int width, int height, const T* buffer, int stride, const PixelFormat& pf,
