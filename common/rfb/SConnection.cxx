...
SConnection::SConnection(AccessRights accessRights_)
    : readyForSetColourMapEntries(false), is(nullptr), os(nullptr), reader_(nullptr), writer_(nullptr),
      ssecurity(nullptr), authFailureTimer(this, &SConnection::handleAuthFailureTimeout),
      state_(RFBSTATE_UNINITIALISED), preferredEncoding(encodingRaw), accessRights(accessRights_),
      hasRemoteClipboard(false), hasLocalClipboard(false), unsolicitedClipboardAttempt(false) {
...
bool SConnection::processMsg() {
  ...
  case RFBSTATE_SECURITY_TYPE:
    return processSecurityTypeMsg();
    break;
  case RFBSTATE_SECURITY:
    return processSecurityMsg();
    break;
  case RFBSTATE_SECURITY_FAILURE:
    return processSecurityFailure();
    break;
...
bool SConnection::processVersionMsg() {
  ...
  vlog.info("Client needs protocol version %d.%d", client.majorVersion, client.minorVersion);

  if (client.majorVersion != 3) {
    // unknown protocol version
    failConnection(core::format("Client needs protocol version %d.%d, server has %d.%d", client.majorVersion,
                                client.minorVersion, defaultMajorVersion, defaultMinorVersion));
  }

  if (client.minorVersion != 3 && client.minorVersion != 7 && client.minorVersion != 8) {
    vlog.error("Client uses unofficial protocol version %d.%d", client.majorVersion, client.minorVersion);
    ...
    vlog.error("Assuming compatibility with version %d.%d", client.majorVersion, client.minorVersion);
  }
  ...
  if (client.isVersion(3, 3)) {
    ...
    if (i == secTypes.end()) {
      failConnection(
          core::format("No supported security type for %d.%d client", client.majorVersion, client.minorVersion));
    }
...
void SConnection::handleAuthFailureTimeout(core::Timer* /*t*/) {
  ...
  try {
    ...
    if (!client.beforeVersion(3, 8)) { // 3.8 onwards have failure message
      ...
      os->writeBytes((const uint8_t*)authFailureMsg.data(), authFailureMsg.size());
    }
    os->flush();
  } catch (std::exception& e) {
    close(e.what());
...
void SConnection::setEncodings(int nEncodings, const int32_t* encodings) {
  bool firstFence, firstContinuousUpdates, firstLEDState, firstQEMUKeyEvent, firstExtMouseButtonsEvent;
  ...
  // Log full SetEncodings list for debugging and client comparison
  {
    ...
    for (int i = 0; i < nEncodings; i++) {
      if (encodings[i] == pseudoEncodingLastRect)
        hasLastRect = true;
      if (encodings[i] == pseudoEncodingPersistentCache)
        hasPersistentCache = true;
    }
    vlog.info("  Key encodings: LastRect=%d PersistentCache=%d", hasLastRect, hasPersistentCache);
  }
  ...
  vlog.debug("Client encodings: PersistentCache=%d", client.supportsEncoding(pseudoEncodingPersistentCache));
  ...
  if (client.supportsEncoding(pseudoEncodingExtendedClipboard)) {
    uint32_t sizes[] = {0};
    writer()->writeClipboardCaps(rfb::clipboardUTF8 | rfb::clipboardRequest | rfb::clipboardPeek |
                                     rfb::clipboardNotify | rfb::clipboardProvide,
                                 sizes);
  }
}
...
void SConnection::handleClipboardProvide(uint32_t flags, const size_t* lengths, const uint8_t* const* data) {
  if (!(flags & rfb::clipboardUTF8)) {
...
void SConnection::handlePersistentCacheQuery(const std::vector<uint64_t>& /*cacheIds*/) {
...
void SConnection::handlePersistentHashList(uint32_t /*sequenceId*/, uint16_t /*totalChunks*/, uint16_t /*chunkIndex*/,
...
void SConnection::desktopReady() {
  if (state() != RFBSTATE_CLIENT_READY)
    return;
  writer_->writeServerInit(client.width(), client.height(), client.pf(), client.name());
...
void SConnection::framebufferUpdateRequest(const core::Rect& /*r*/, bool /*incremental*/) {
...
void SConnection::fence(uint32_t flags, unsigned len, const uint8_t data[]) {
...
void SConnection::enableContinuousUpdates(bool /*enable*/, int /*x*/, int /*y*/, int /*w*/, int /*h*/) {}
...
void SConnection::requestClipboard() {
  ...
  if (client.supportsEncoding(pseudoEncodingExtendedClipboard) && (client.clipboardFlags() & rfb::clipboardRequest))
...
void SConnection::announceClipboard(bool available) {
  ...
  if (client.supportsEncoding(pseudoEncodingExtendedClipboard)) {
    ...
    if (available && (client.clipboardSize(rfb::clipboardUTF8) > 0) &&
...
void SConnection::sendClipboardData(const char* data) {
  ...
  if (client.supportsEncoding(pseudoEncodingExtendedClipboard) && (client.clipboardFlags() & rfb::clipboardProvide)) {
