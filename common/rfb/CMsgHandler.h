...
public:
  ...
  virtual void setExtendedDesktopSize(unsigned reason, unsigned result, int w, int h, const ScreenSet& layout) = 0;
  virtual void setCursor(int width, int height, const core::Point& hotspot, const uint8_t* data) = 0;
  ...
  virtual void serverInit(int width, int height, const PixelFormat& pf, const char* name) = 0;

  virtual bool readAndDecodeRect(const core::Rect& r, int encoding, ModifiablePixelBuffer* pb,
                                 const ServerParams* serverOverride = nullptr) = 0;

  virtual void framebufferUpdateStart() = 0;
  virtual void framebufferUpdateEnd() = 0;
  virtual bool dataRect(const core::Rect& r, int encoding, const ServerParams* serverOverride = nullptr) = 0;

  virtual void setColourMapEntries(int firstColour, int nColours, uint16_t* rgbs) = 0;
  virtual void bell() = 0;
  ...
  // Uses the shared 16-byte CacheKey identity so
  ...
  virtual void handlePersistentCachedRectWithOffset(const core::Rect& r, const CacheKey& key, uint16_t ox, uint16_t oy,
                                                    uint16_t cachedW, uint16_t cachedH) = 0;
  ...
  virtual bool supportsNativeFormatCache() const {
    return false;
  }
