...
bool HextileDecoder::readRect(const core::Rect& r, rdr::InStream* is, const ServerParams& server, rdr::OutStream* os) {
  core::Rect t;
...
void HextileDecoder::decodeRect(const core::Rect& r, const uint8_t* buffer, size_t buflen, const ServerParams& server,
                                ModifiablePixelBuffer* pb) {
  rdr::MemInStream is(buffer, buflen);
  const PixelFormat& pf = server.pf();
  switch (pf.bpp) {
  case 8:
    hextileDecode<uint8_t>(r, &is, pf, pb);
    break;
  case 16:
    hextileDecode<uint16_t>(r, &is, pf, pb);
    break;
  case 32:
    hextileDecode<uint32_t>(r, &is, pf, pb);
    break;
  }
}

template <class T> inline T HextileDecoder::readPixel(rdr::InStream* is) {
  if (sizeof(T) == 1)
...
template <class T>
void HextileDecoder::hextileDecode(const core::Rect& r, rdr::InStream* is, const PixelFormat& pf,
                                   ModifiablePixelBuffer* pb) {
  core::Rect t;
  T bg = 0;
  T fg = 0;
