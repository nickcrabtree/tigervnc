...
static unsigned char ffdhe2048[] = "-----BEGIN DH PARAMETERS-----\n"
...
static const gnutls_datum_t ffdhe_pkcs3_param = {ffdhe2048, sizeof(ffdhe2048)};
#endif

using namespace rfb;

core::StringParameter SSecurityTLS::X509_CertFile("X509Cert", "Path to the X509 certificate in PEM format", "");
...
SSecurityTLS::SSecurityTLS(SConnection* sc_, bool _anon)
    : SSecurity(sc_), session(nullptr), anon_cred(nullptr), cert_cred(nullptr), anon(_anon), tlssock(nullptr),
...
void SSecurityTLS::setParams() {
  ...
  ret = gnutls_dh_params_import_pkcs3(dh_params, &ffdhe_pkcs3_param, GNUTLS_X509_FMT_PEM);
  if (ret != GNUTLS_E_SUCCESS)
  ...
  if (anon) {
    ...
    ret = gnutls_certificate_set_x509_key_file(cert_cred, X509_CertFile, X509_KeyFile, GNUTLS_X509_FMT_PEM);
    if (ret != GNUTLS_E_SUCCESS)
