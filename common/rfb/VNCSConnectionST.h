...
public:
  VNCSConnectionST(VNCServerST* server_, network::Socket* s, bool reverse, AccessRights ar);
  ...
  const char* getPeerEndpoint() const {
...
private:
  ...
  void setDesktopSize(int fb_width, int fb_height, const ScreenSet& layout) override;
  ...
  void handlePersistentCacheHashReport(const CacheKey& canonicalKey, const CacheKey& actualKey) override;
