...
PixelBuffer::PixelBuffer(const PixelFormat& pf, int w, int h) : format(pf), width_(0), height_(0) {
  setSize(w, h);
}
...
void PixelBuffer::getImage(void* imageBuf, const core::Rect& r, int outStride) const {
  int inStride;
  const uint8_t* data;
  ...
  if (!r.enclosed_by(getRect()))
    throw std::out_of_range(core::format("Source rect %dx%d at %d,%d exceeds framebuffer %dx%d", r.width(), r.height(),
...
void PixelBuffer::getImage(const PixelFormat& pf, void* imageBuf, const core::Rect& r, int stride) const {
  ...
  pf.bufferFromBuffer((uint8_t*)imageBuf, format, srcBuffer, r.width(), r.height(), stride, srcStride);
}

void PixelBuffer::setSize(int width, int height) {
  if ((width < 0) || (width > maxPixelBufferWidth))
    throw std::out_of_range(core::format("Invalid PixelBuffer width of %d pixels requested", width));
  if ((height < 0) || (height > maxPixelBufferHeight))
    throw std::out_of_range(core::format("Invalid PixelBuffer height of %d pixels requested", height));
...
ModifiablePixelBuffer::ModifiablePixelBuffer(const PixelFormat& pf, int w, int h) : PixelBuffer(pf, w, h) {}
...
void ModifiablePixelBuffer::fillRect(const core::Rect& r, const void* pix) {
  ...
  if (!r.enclosed_by(getRect()))
    throw std::out_of_range(core::format("Destination rect %dx%d at %d,%d exceeds framebuffer %dx%d", r.width(),
                                         r.height(), r.tl.x, r.tl.y, width(), height()));
...
void ModifiablePixelBuffer::imageRect(const core::Rect& r, const void* pixels, int srcStride) {
  uint8_t* dest;
  int destStride;
...
void ModifiablePixelBuffer::copyRect(const core::Rect& rect, const core::Point& move_by_delta) {
  ...
  if (!drect.enclosed_by(getRect()))
    throw std::out_of_range(core::format("Destination rect %dx%d at %d,%d exceeds framebuffer %dx%d", drect.width(),
                                         drect.height(), drect.tl.x, drect.tl.y, width(), height()));

  srect = drect.translate(move_by_delta.negate());
  if (!srect.enclosed_by(getRect()))
    throw std::out_of_range(core::format("Source rect %dx%d at %d,%d exceeds framebuffer %dx%d", srect.width(),
                                         srect.height(), srect.tl.x, srect.tl.y, width(), height()));
...
void ModifiablePixelBuffer::fillRect(const PixelFormat& pf, const core::Rect& dest, const void* pix) {
  uint8_t buf[4];
  format.bufferFromBuffer(buf, pf, (const uint8_t*)pix, 1);
  fillRect(dest, buf);
}

void ModifiablePixelBuffer::imageRect(const PixelFormat& pf, const core::Rect& dest, const void* pixels, int stride) {
  uint8_t* dstBuffer;
  int dstStride;

  if (!dest.enclosed_by(getRect()))
    throw std::out_of_range(core::format("Destination rect %dx%d at %d,%d exceeds framebuffer %dx%d", dest.width(),
                                         dest.height(), dest.tl.x, dest.tl.y, width(), height()));
  ...
  if (pf != format) {
    ...
    vlog.debug("imageRect FORMAT CONVERSION: rect=[%d,%d-%d,%d] src=[%s] dst=[%s]", dest.tl.x, dest.tl.y, dest.br.x,
               dest.br.y, srcFmt, dstFmt);
  }

  dstBuffer = getBufferRW(dest, &dstStride);
  format.bufferFromBuffer(dstBuffer, pf, (const uint8_t*)pixels, dest.width(), dest.height(), dstStride, stride);
  commitBufferRW(dest);
}

// -=- Simple pixel buffer with a continuous block of memory

FullFramePixelBuffer::FullFramePixelBuffer(const PixelFormat& pf, int w, int h, uint8_t* data_, int stride_)
...
uint8_t* FullFramePixelBuffer::getBufferRW(const core::Rect& r, int* stride_) {
  if (!r.enclosed_by(getRect()))
    throw std::out_of_range(core::format("Pixel buffer request %dx%d at %d,%d exceeds framebuffer %dx%d", r.width(),
                                         r.height(), r.tl.x, r.tl.y, width(), height()));
...
void FullFramePixelBuffer::setBuffer(int width, int height, uint8_t* data_, int stride_) {
  if ((width < 0) || (width > maxPixelBufferWidth))
    throw std::out_of_range(core::format("Invalid PixelBuffer width of %d pixels requested", width));
  if ((height < 0) || (height > maxPixelBufferHeight))
    throw std::out_of_range(core::format("Invalid PixelBuffer height of %d pixels requested", height));
  if ((stride_ < 0) || (stride_ > maxPixelBufferStride) || (stride_ < width))
    throw std::invalid_argument(core::format("Invalid PixelBuffer stride of %d pixels requested", stride_));
  if ((width != 0) && (height != 0) && (data_ == nullptr))
    throw std::logic_error(core::format("PixelBuffer requested without a valid memory area"));
...
ManagedPixelBuffer::ManagedPixelBuffer() : data_(nullptr), datasize(0) {}
