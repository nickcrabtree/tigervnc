...
std::unique_ptr<CacheCoordinator> CacheCoordinator::create(const std::string& cacheDir,
  ...
  return std::unique_ptr<CacheCoordinator>(
      new StandaloneCoordinator(cacheDir, std::move(indexUpdateCb), std::move(writeRequestCb)));
#else
  ...
  if (flock(lockFd, LOCK_EX | LOCK_NB) == 0) {
    ...
    std::unique_ptr<CacheCoordinator> master(
        new MasterCoordinator(cacheDir, std::move(indexUpdateCb), std::move(writeRequestCb)));
  ...
  return std::unique_ptr<CacheCoordinator>(
      new SlaveCoordinator(cacheDir, std::move(indexUpdateCb), std::move(writeRequestCb)));
...
MasterCoordinator::MasterCoordinator(const std::string& cacheDir, IndexUpdateCallback indexUpdateCb,
                                     WriteRequestCallback writeRequestCb)
    : CacheCoordinator(cacheDir, std::move(indexUpdateCb), std::move(writeRequestCb)), lockFd_(-1), listenFd_(-1),
...
void MasterCoordinator::handleWriteRequest(int clientFd, const CoordMessage& msg) {
  ...
  std::vector<uint8_t> payload(msg.payload().begin() + payloadOffset,
...
void MasterCoordinator::removeClient(int clientFd) {
  std::lock_guard<std::mutex> lock(clientsMutex_);
  close(clientFd);
  clientFds_.erase(std::remove(clientFds_.begin(), clientFds_.end(), clientFd), clientFds_.end());
  clientBuffers_.erase(clientFd);

  std::lock_guard<std::mutex> statsLock(statsMutex_);
  if (stats_.connectedSlaves > 0)
    stats_.connectedSlaves--;
}

bool MasterCoordinator::requestWrite(const WireIndexEntry&, const std::vector<uint8_t>&, WireIndexEntry&) {
...
SlaveCoordinator::SlaveCoordinator(const std::string& cacheDir, IndexUpdateCallback indexUpdateCb,
                                   WriteRequestCallback writeRequestCb)
    : CacheCoordinator(cacheDir, std::move(indexUpdateCb), std::move(writeRequestCb)), socketFd_(-1), running_(false),
      stopRequested_(false), writeAckReceived_(false), lastWriteSuccess_(false) {
  memset(&stats_, 0, sizeof(stats_));
}
...
void SlaveCoordinator::handleWelcome(const CoordMessage& msg) {
  ...
  vlog.info("Connected to master (PID %d), protocol v%d, %u entries", header.masterPid, header.protocolVersion,
            header.entryCount);
  ...
  if (header.entryCount > 0 && indexUpdateCallback_) {
    ...
    for (uint32_t i = 0; i < header.entryCount; i++) {
      ...
      if (!msg.readStruct(offset, entry))
        break;
...
bool SlaveCoordinator::requestWrite(const WireIndexEntry& entry, const std::vector<uint8_t>& payload,
                                    WireIndexEntry& resultEntry) {
...
bool SlaveCoordinator::queryIndex(const uint8_t* hash, uint16_t width, uint16_t height, WireIndexEntry& /*entry*/) {
  if (!running_ || socketFd_ < 0)
    return false;
