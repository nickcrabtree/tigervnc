...
enum class TileCacheState { NotCacheable = 0, Hit, InitCandidate };
...
public:
  ...
  virtual TileCacheState classifyTile(const core::Rect& tileRect, const PixelBuffer* pb) = 0;
};
...
bool findLargestHitRectangle(const std::vector<TileInfo>& tiles, int tilesX, int tilesY, int minTiles, MaxRect& outMax);
