...
public:
  ...
  static int parse(const uint8_t* data, size_t len, CoordMessage& out) {
    if (len < 5)
      return 0; // Need at least length + type

    uint32_t netLen;
    memcpy(&netLen, data, 4);
    uint32_t msgLen = ntohl(netLen);

    if (msgLen < 1 || msgLen > 64 * 1024 * 1024)
  ...
  template <typename T> void appendStruct(const T& s) {
    const uint8_t* p = reinterpret_cast<const uint8_t*>(&s);
    payload_.insert(payload_.end(), p, p + sizeof(T));
  }

  // Convenience: read struct from payload at offset
  template <typename T> bool readStruct(size_t offset, T& out) const {
