...
PixelFormat::PixelFormat(int b, int d, bool e, bool t, int rm, int gm, int bm, int rs, int gs, int bs)
    : bpp(b), depth(d), trueColour(t), bigEndian(e), redMax(rm), greenMax(gm), blueMax(bm), redShift(rs),
      greenShift(gs), blueShift(bs) {
  if (!isSane())
...
PixelFormat::PixelFormat()
    : bpp(8), depth(8), trueColour(true), bigEndian(false), redMax(7), greenMax(7), blueMax(3), redShift(0),
      greenShift(3), blueShift(6) {
  updateState();
}
...
void PixelFormat::bufferFromRGB(uint8_t* dst, const uint8_t* src, int w, int stride, int h) const {
  if (is888()) {
...
void PixelFormat::rgbFromBuffer(uint8_t* dst, const uint8_t* src, int w, int stride, int h) const {
  if (is888()) {
    // Optimised common case
    const uint8_t *r, *g, *b;
...
void PixelFormat::bufferFromBuffer(uint8_t* dst, const PixelFormat& srcPF, const uint8_t* src, int pixels) const {
  bufferFromBuffer(dst, srcPF, src, pixels, 1, pixels, pixels);
}

#define IS_ALIGNED(v, a) (((intptr_t)v & (a - 1)) == 0)

void PixelFormat::bufferFromBuffer(uint8_t* dst, const PixelFormat& srcPF, const uint8_t* src, int w, int h,
  ...
  if (*this == srcPF) {
    ...
    switch (bpp) {
    case 8:
      directBufferFromBufferFrom888((uint8_t*)dst, srcPF, src, w, h, dstStride, srcStride);
      break;
    case 16:
      directBufferFromBufferFrom888((uint16_t*)dst, srcPF, src, w, h, dstStride, srcStride);
      break;
    case 32:
      directBufferFromBufferFrom888((uint32_t*)dst, srcPF, src, w, h, dstStride, srcStride);
      break;
    }
    ...
    switch (srcPF.bpp) {
    case 8:
      directBufferFromBufferTo888(dst, srcPF, (uint8_t*)src, w, h, dstStride, srcStride);
      break;
    case 16:
      directBufferFromBufferTo888(dst, srcPF, (uint16_t*)src, w, h, dstStride, srcStride);
      break;
    case 32:
      directBufferFromBufferTo888(dst, srcPF, (uint32_t*)src, w, h, dstStride, srcStride);
      break;
    }
  } else {
    // Generic code
...
void PixelFormat::print(char* str, int len) const {
  ...
  if (blueShift == 0 && greenShift > blueShift && redShift > greenShift && blueMax == (1 << greenShift) - 1 &&
      greenMax == (1 << (redShift - greenShift)) - 1 && redMax == (1 << (depth - redShift)) - 1) {
  ...
  if (redShift == 0 && greenShift > redShift && blueShift > greenShift && redMax == (1 << greenShift) - 1 &&
      greenMax == (1 << (blueShift - greenShift)) - 1 && blueMax == (1 << (depth - blueShift)) - 1) {
...
template <class T>
void PixelFormat::directBufferFromBufferFrom888(T* dst, const PixelFormat& srcPF, const uint8_t* src, int w, int h,
                                                int dstStride, int srcStride) const {
  const uint8_t *r, *g, *b;
  int dstPad, srcPad;
...
template <class T>
void PixelFormat::directBufferFromBufferTo888(uint8_t* dst, const PixelFormat& srcPF, const T* src, int w, int h,
                                              int dstStride, int srcStride) const {
  uint8_t *r, *g, *b, *x;
  int dstPad, srcPad;
