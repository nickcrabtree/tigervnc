...
public:
  ...
  void writeUpdate(const UpdateInfo& ui, const PixelBuffer* pb, const RenderedCursor* renderedCursor);

  void writeLosslessRefresh(const core::Region& req, const PixelBuffer* pb, const RenderedCursor* renderedCursor,
  ...
  void setUsePersistentCache(bool enable) {
    usePersistentCache = enable;
  }
  void setNativeFormatCacheSupported(bool enable) {
    nativeFormatCacheSupported = enable;
  }

protected:
  void handleTimeout(core::Timer* t) override;

  void doUpdate(bool allowLossy, const core::Region& changed, const core::Region& copied, const core::Point& copy_delta,
                const PixelBuffer* pb, const RenderedCursor* renderedCursor);
  ...
  void selectEncoderForRect(const core::Rect& rect, const PixelBuffer* pb, PixelBuffer*& ppb, struct RectInfo* info,
                            EncoderType& type);
  ...
  bool checkSolidTile(const core::Rect& r, const uint8_t* colourValue, const PixelBuffer* pb);
  void extendSolidAreaByBlock(const core::Rect& r, const uint8_t* colourValue, const PixelBuffer* pb, core::Rect* er);
  void extendSolidAreaByPixel(const core::Rect& r, const core::Rect& sr, const uint8_t* colourValue,
                              const PixelBuffer* pb, core::Rect* er);

  PixelBuffer* preparePixelBuffer(const core::Rect& rect, const PixelBuffer* pb, bool convert);

  bool analyseRect(const PixelBuffer* pb, struct RectInfo* info, int maxColours);

protected:
  // Templated, optimised methods
  template <class T>
  inline bool checkSolidTile(int width, int height, const T* buffer, int stride, const T colourValue);
  template <class T>
  inline bool analyseRect(int width, int height, const T* buffer, int stride, struct RectInfo* info, int maxColours);
...
protected:
  ...
  public:
    ...
    void update(const PixelFormat& pf, int width, int height, const uint8_t* data_, int stride);
