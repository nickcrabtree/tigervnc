...
int rfb::encodingNum(const char* name) {
  if (strcasecmp(name, "raw") == 0)
    return encodingRaw;
  if (strcasecmp(name, "copyRect") == 0)
    return encodingCopyRect;
  if (strcasecmp(name, "RRE") == 0)
    return encodingRRE;
  if (strcasecmp(name, "CoRRE") == 0)
    return encodingCoRRE;
  if (strcasecmp(name, "hextile") == 0)
    return encodingHextile;
  if (strcasecmp(name, "ZRLE") == 0)
    return encodingZRLE;
  if (strcasecmp(name, "Tight") == 0)
    return encodingTight;
#ifdef HAVE_H264
  if (strcasecmp(name, "H.264") == 0)
    return encodingH264;
#endif
