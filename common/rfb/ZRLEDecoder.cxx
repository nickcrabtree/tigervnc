...
template <class T> static inline T readPixel(rdr::ZlibInStream* zis) {
  if (sizeof(T) == 1)
...
bool ZRLEDecoder::readRect(const core::Rect& /*r*/, rdr::InStream* is, const ServerParams& /*server*/,
                           rdr::OutStream* os) {
  uint32_t len;
...
void ZRLEDecoder::decodeRect(const core::Rect& r, const uint8_t* buffer, size_t buflen, const ServerParams& server,
                             ModifiablePixelBuffer* pb) {
  rdr::MemInStream is(buffer, buflen);
  const rfb::PixelFormat& pf = server.pf();
  switch (pf.bpp) {
  case 8:
    zrleDecode<uint8_t>(r, &is, pf, pb);
    break;
  case 16:
    zrleDecode<uint16_t>(r, &is, pf, pb);
    break;
  case 32:
    zrleDecode<uint32_t>(r, &is, pf, pb);
    break;
  }
}

template <class T>
void ZRLEDecoder::zrleDecode(const core::Rect& r, rdr::InStream* is, const PixelFormat& pf, ModifiablePixelBuffer* pb) {
  ...
  bool isLowCPixel = (sizeof(T) == 4) && (pf.depth <= 24) &&
                     ((fitsInLS3Bytes && pf.isLittleEndian()) || (fitsInMS3Bytes && pf.isBigEndian()));
  bool isHighCPixel = (sizeof(T) == 4) && (pf.depth <= 24) &&
                      ((fitsInLS3Bytes && pf.isBigEndian()) || (fitsInMS3Bytes && pf.isLittleEndian()));
