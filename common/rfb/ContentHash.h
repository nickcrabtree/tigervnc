...
public:
  ...
  static std::vector<uint8_t> computeRect(const PixelBuffer* pb, const core::Rect& r) {
    if (!pb)
  ...
  static std::vector<BorderedRegion> detectBorderedRegions(const PixelBuffer* pb, int minBorderWidth = 5,
                                                           int minArea = 50000);
};
