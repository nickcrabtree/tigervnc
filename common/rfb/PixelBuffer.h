...
public:
  ...
  //   specific format.
  void getImage(const PixelFormat& pf, void* imageBuf, const core::Rect& r, int stride = 0) const;
...
public:
  ...
  void copyRect(const core::Rect& dest, const core::Point& move_by_delta);
  ...
  void fillRect(const PixelFormat& pf, const core::Rect& dest, const void* pix);
  void imageRect(const PixelFormat& pf, const core::Rect& dest, const void* pixels, int stride = 0);
...
public:
  FullFramePixelBuffer(const PixelFormat& pf, int width, int height, uint8_t* data_, int stride);
