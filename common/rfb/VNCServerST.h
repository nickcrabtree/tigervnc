...
public:
  ...
  void addSocket(network::Socket* sock, bool outgoing = false, AccessRights ar = AccessDefault) override;
  ...
  void approveConnection(network::Socket* sock, bool accept, const char* reason) override;
  void closeClients(const char* reason) override {
    closeClients(reason, nullptr);
  }
  ...
  const ScreenSet& getScreenLayout() const {
    return screenLayout;
  }
  const Cursor* getCursor() const {
    return cursor;
  }
  const core::Point& getCursorPos() const {
    return cursorPos;
  }
  const char* getName() const {
    return name.c_str();
  }
  unsigned getLEDState() const {
    return ledState;
  }
  bool isDesktopReady() const {
    return desktopStarted;
  }

  // Event handlers
  void keyEvent(uint32_t keysym, uint32_t keycode, bool down);
  void pointerEvent(VNCSConnectionST* client, const core::Point& pos, uint16_t buttonMask);
  ...
  unsigned int setDesktopSize(VNCSConnectionST* requester, int fb_width, int fb_height, const ScreenSet& layout);
