...
class Cursor {
public:
  Cursor(int width, int height, const core::Point& hotspot, const uint8_t* data);
  ...
  const core::Point& hotspot() const {
    return hotspot_;
  };
...
public:
  ...
  core::Rect getEffectiveRect() const {
    return buffer.getRect(offset);
  }

  const uint8_t* getBuffer(const core::Rect& r, int* stride) const override;

  void update(PixelBuffer* framebuffer, Cursor* cursor, const core::Point& pos);
