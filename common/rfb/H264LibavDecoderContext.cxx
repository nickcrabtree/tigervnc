...
H264LibavDecoderContext::H264LibavDecoderContext(const core::Rect& r) : H264DecoderContext(r) {
  sws = nullptr;
...
void H264LibavDecoderContext::decode(const uint8_t* h264_in_buffer, uint32_t len, ModifiablePixelBuffer* pb) {
  ...
  while (len) {
    ret = av_parser_parse2(parser, avctx, &packet->data, &packet->size, h264_work_buffer, len, AV_NOPTS_VALUE,
                           AV_NOPTS_VALUE, 0);
  ...
  sws = sws_getCachedContext(sws, frame->width, frame->height, avctx->pix_fmt, frame->width, frame->height,
  ...
  sws_getColorspaceDetails(sws, (int**)&inTable, &inFull, (int**)&outTable, &outFull, &brightness, &contrast,
                           &saturation);
  ...
  sws_setColorspaceDetails(sws, inTable, inFull, outTable, outFull, brightness, contrast, saturation);
  ...
  sws_scale(sws, frame->data, frame->linesize, 0, frame->height, rgbFrame->data, rgbFrame->linesize);
