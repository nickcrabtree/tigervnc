...
Socket::Socket(int fd) : instream(nullptr), outstream(nullptr), isShutdown_(false), queryConnection(false) {
  initSockets();
  setFd(fd);
}

Socket::Socket() : instream(nullptr), outstream(nullptr), isShutdown_(false), queryConnection(false) {
  initSockets();
}
...
SocketListener::SocketListener(int fd_) : fd(fd_), filter(nullptr) {
  initSockets();
}

SocketListener::SocketListener() : fd(-1), filter(nullptr) {
  initSockets();
}
